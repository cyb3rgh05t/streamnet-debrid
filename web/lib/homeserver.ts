...
export function parseHomeServerConnectionJson(
  ...
  const rawList: unknown[] = Array.isArray(root)
    ...
    : root &&
        typeof root === "object" &&
        Array.isArray((root as { connections?: unknown[] }).connections)
  ...
  return rawList
    .map((raw): HomeServerConfig | null => {
      ...
      const collections: HomeServerCollectionConfig[] = Array.isArray(
        r.collections,
      )
      ...
      return {
        ...
        lastConnectedAt:
          typeof r.lastConnectedAt === "number" ? r.lastConnectedAt : undefined,
      };
    })
...
export function serializeHomeServerConnectionJson(
  servers: HomeServerConfig[] | undefined,
): string {
...
const AUTH_HEADER =
  'MediaBrowser Client="StreamNet Web", Device="Web", DeviceId="streamnet-web", Version="1.0.0"';
...
function homeServerKey(server: HomeServerConfig): string {
  return (
    ...
    server.id.trim() ||
    `${webTypeToApkKind(server.type)}:${trimUrl(server.url)}`
  );
}
...
export async function buildHomeServerCatalogConfigs(
  ...
  const savedHomeCatalogs = savedCatalogs.filter(
    (catalog) => catalog.sourceType === "home-server",
  );
  const savedById = new Map(
    savedHomeCatalogs.map((catalog) => [catalog.id, catalog]),
  );
  ...
  for (const server of servers) {
    for (const collection of server.collections ?? []) {
      ...
      const existing =
        savedById.get(identity.id) ?? savedBySourceRef.get(identity.sourceRef);
      const title =
        existing?.name ||
        existing?.title ||
      ...
      generated.push({
        ...
        mediaType: normalizedType.includes("movie")
          ? "movie"
  ...
  const generatedById = new Map(
    generated.map((catalog) => [catalog.id, catalog]),
  );
...
async function proxiedPost<T>(
  ...
  return jsonRequest<T>(target.toString(), {
    method: "POST",
...
async function ensureSession(
  ...
  if (server.token) {
    try {
      const me = await proxiedGet<{ Id: string }>(
        `${base}/Users/Me?api_key=${encodeURIComponent(server.token)}`,
        {
    ...
    try {
      const users = await proxiedGet<Array<{ Id: string }>>(
        `${base}/Users?api_key=${encodeURIComponent(server.token)}`,
        {
          "X-Emby-Token": server.token,
        },
        signal,
...
function mapItem(
  ...
  const image = primaryTag
    ? directUrl(
        `${base}/Items/${item.Id}/Images/Primary?maxWidth=500&tag=${primaryTag}&api_key=${token}`,
      )
    : "";
  const backdropTag = item.BackdropImageTags?.[0];
  const backdrop = backdropTag
    ? directUrl(
        `${base}/Items/${item.Id}/Images/Backdrop/0?maxWidth=1280&tag=${backdropTag}&api_key=${token}`,
      )
    : null;
  ...
  return {
    ...
    homeServerUrl:
      mediaType === "movie"
        ? directUrl(
            `${base}/Videos/${item.Id}/stream?static=true&api_key=${token}`,
          )
        : null,
  };
}
...
function plexImage(base: string, token: string, path?: string) {
  return path
    ? directUrl(`${base}${path}?X-Plex-Token=${encodeURIComponent(token)}`)
    : "";
}
...
function plexStreamUrl(base: string, token: string, item: PlexItem) {
  ...
  return part
    ? directUrl(`${base}${part}?X-Plex-Token=${encodeURIComponent(token)}`)
    : null;
}
...
function mapPlexItem(
  ...
  return {
    ...
    homeServerUrl:
      mediaType === "movie" ? plexStreamUrl(base, token, item) : null,
  };
}
...
async function loadPlexRows(
  ...
  const sections = await proxiedGet<{
  ...
  const libraries = (sections?.MediaContainer?.Directory ?? [])
    ...
    .filter((section) => {
      ...
      return server.collections.some(
        (collection) =>
  ...
  const rows = await Promise.all(
    libraries.map(async (library) => {
      const configured = server.collections?.find(
        (collection) => collection.id === library.key,
      );
...
export async function loadHomeServerRows(
  ...
  const rowsPerServer = await Promise.all(
    active.map(async (server) => {
      ...
      try {
        ...
        const rows = await Promise.all(
          libraries.map(async (library) => {
            const configured = server.collections?.find(
              (collection) => collection.id === library.Id,
            );
            if (server.collections?.length && !configured) return null;
            if (configured?.enabled === false) return null;
            const identity = await homeServerCatalogIdentity(server, {
            ...
            const mapped = (items.Items ?? [])
              .map((item) => mapItem(base, token, item, server))
  ...
  const order = new Map(
    catalogOrder.map((catalog, index) => [catalog.id, index]),
  );
  return [...plexRows.flat(), ...rowsPerServer.flat()].sort(
    (left, right) =>
...
export async function testHomeServerConnection(
  ...
  try {
    if (server.type === "plex") {
      ...
      const sections = await proxiedGet<{
        MediaContainer?: { Directory?: PlexSection[]; friendlyName?: string };
      }>(`${base}/library/sections?X-Plex-Token=${encodeURIComponent(token)}`, {
    ...
    if (!session)
      return {
        ok: false,
        error: "Authentication failed — check token or username/password",
      };
    const views = await proxiedGet<{
      Items?: Array<{ Id: string; Name: string; CollectionType?: string }>;
    }>(`${base}/Users/${session.userId}/Views?api_key=${session.token}`).catch(
      () => null,
    );
...
const sourceCache = new Map<
  string,
  { expiresAt: number; sources: StreamSource[] }
>();
...
function homeServerSourceCacheKey(
  ...
  return [
    type,
    sourceServerSignature(servers),
    sourceContentIdentity(target),
    season ?? "",
    episode ?? "",
  ].join("|");
}
  ...
  request = loader()
    .then((sources) => {
      ...
      sourceCache.set(key, {
        ...
        expiresAt:
          Date.now() +
          (sources.length ? SOURCE_CACHE_TTL_MS : EMPTY_SOURCE_CACHE_TTL_MS),
      });
      return sources;
    })
    .finally(() => {
...
function scoreCandidate(target: MatchTarget, c: Candidate): number {
  ...
  const providers = Object.fromEntries(
    Object.entries(c.providerIds).map(([k, v]) => [k.toLowerCase(), v]),
  );
  const imdb = target.imdbId?.trim().toLowerCase();
  if (imdb && providers.imdb?.toLowerCase() === imdb) score += 1000;
  if (target.tmdbId != null && Number(providers.tmdb) === target.tmdbId)
    score += 900;
...
function isLikelySameVersion(
  target: MatchTarget,
  candidate: Candidate,
): boolean {
  if (
    !normalizeTitle(target.title) ||
    normalizeTitle(target.title) !== normalizeTitle(candidate.title)
  )
    return false;
...
export function jellyfinSourceMedia(
  source: JellyfinMediaSource,
): NonNullable<StreamSource["media"]> {
  ...
  const audio =
    streams.find(
      (s) =>
        ...
        s.Index === source.DefaultAudioStreamIndex,
    ) ??
  ...
  return {
    ...
    hdr:
      video?.DvProfile || video?.DolbyVisionProfile
        ? "Dolby Vision"
        : range && range !== "Unknown"
          ? range
...
async function jellyfinFindItems(
  ...
  return scored
    .filter(
      ({ candidate, score }) =>
        score === bestScore || isLikelySameVersion(target, candidate),
    )
...
async function jellyfinFindItem(
  ...
  return (
    (await jellyfinFindItems(server, session, target, itemTypes))[0] ?? null
  );
}
...
async function jellyfinItemSources(
  ...
  const playbackInfo = await proxiedPost<{
    MediaSources?: JellyfinFullItem["MediaSources"];
  }>(
  ...
  for (const [mediaIndex, ms] of mediaSources.entries()) {
    ...
    const quality =
      qualityLabel(videoStream?.Width, videoStream?.Height) || "Direct";
    ...
    seen.add(url);
    out.push({
      source: [label, quality, container.toUpperCase()]
        .filter(Boolean)
        .join(" "),
      addonName: label,
...
export function plexSourceMedia(
  media: PlexMedia,
  part: NonNullable<PlexMedia["Part"]>[number],
): NonNullable<StreamSource["media"]> {
  const streams = part.Stream ?? [];
  const video = streams.find((s) => s.streamType === 1);
  const audio =
    streams.find((s) => s.streamType === 2 && s.selected) ??
    streams.find((s) => s.streamType === 2 && s.default) ??
  ...
  return {
    ...
    hdr:
      video?.DOVIPresent || video?.DOVIProfile
        ? "Dolby Vision"
        : video?.videoRange ||
          (video?.colorTrc === "smpte2084"
            ? "HDR10"
            : video?.colorTrc === "arib-std-b67"
              ? "HLG"
              : undefined),
  };
}
...
async function plexFindMetadataMatches(
  ...
  const scored = responses
    ...
    .map((meta) => {
      const candidate = {
        title: meta.title,
        year: meta.year,
        providerIds: plexProviderIds(meta),
      };
  ...
  return scored
    ...
    .filter(
      ({ meta }, index, all) =>
        all.findIndex((entry) => entry.meta.ratingKey === meta.ratingKey) ===
        index,
    )
    .map(({ meta }) => meta);
}
...
async function plexMetadataSources(
  server: HomeServerConfig,
  meta: PlexMetadata,
): Promise<StreamSource[]> {
  const base = trimUrl(server.url);
    ...
    for (const [partIndex, part] of (media.Part ?? []).entries()) {
      ...
      const quality = media.videoResolution
        ? media.videoResolution === "4k"
          ? "4K"
          : `${media.videoResolution}p`.replace("pp", "p")
        : "Direct";
      out.push({
        source: [label, quality, container.toUpperCase()]
          .filter(Boolean)
        ...
        homeServer: {
          serverId: server.id,
          itemId: hydrated.ratingKey,
          mediaSourceId: media.id == null ? undefined : String(media.id),
          mediaIndex,
          partIndex,
        },
        behaviorHints: {
          cached: true,
          filename: hydrated.title,
          videoSize: part.size && part.size > 0 ? part.size : null,
        },
...
function usableServers(servers: HomeServerConfig[]): HomeServerConfig[] {
  return servers.filter(
    (s) => s.enabled && s.url && (s.type === "plex" ? Boolean(s.token) : true),
  );
}
      ...
      active.map(async (server) => {
        try {
          if (server.type === "plex") {
            ...
            return (
              await Promise.all(
                matches.map((meta) => plexMetadataSources(server, meta)),
              )
            ).flat();
          }
          const session = await ensureSession(server);
          if (!session) return [];
          const items = await jellyfinFindItems(
            server,
            session,
            target,
            "Movie",
          ...
          return (
            await Promise.all(
              items.map((item) => jellyfinItemSources(server, session, item)),
            )
          ).flat();
        } catch {
          return [];
...
export async function resolveHomeServerEpisodeSources(
  ...
  const active = usableServers(servers);
  if (!active.length) return [];
  const cacheKey = homeServerSourceCacheKey(
    "episode",
    active,
    target,
    season,
    ...
    const perServer = await Promise.all(
      active.map(async (server) => {
        try {
          ...
          const session = await ensureSession(server);
          if (!session) return [];
          const series = await jellyfinFindItem(
            server,
            session,
            target,
            "Series",
          ...
          const episodes = (epRes?.Items ?? []).filter(
            (item) =>
          ...
          return (
            await Promise.all(
              episodes.map((item) =>
                jellyfinItemSources(server, session, item),
              ),
            )
          ).flat();
        } catch {
...
async function plexEpisodeSources(
  ...
  const res = await proxiedGet<{
    MediaContainer?: {
      Metadata?: Array<PlexMetadata & { index?: number; parentIndex?: number }>;
    };
  }>(
  ...
  return (
    await Promise.all(episodes.map((item) => plexMetadataSources(server, item)))
  ).flat();
}
...
export function homeServerLibraryMediaFilter(
  mediaType: HomeServerLibraryOption["mediaType"] | string | undefined,
): "movie" | "tv" | "all" {
  const normalized = mediaType?.trim().toLowerCase();
  if (["movie", "movies", "film", "films"].includes(normalized ?? ""))
    return "movie";
  if (
    ["tv", "show", "shows", "series", "tvshow", "tvshows"].includes(
      normalized ?? "",
    )
  )
    return "tv";
...
function parseLibraryValue(
  ...
  try {
    return {
      ...
      libraryId: decodeURIComponent(match[2]),
    };
  } catch {
        ...
        if (server.type === "plex") {
          ...
          return (res?.MediaContainer?.Directory ?? [])
            ...
            .map((s) => ({
              ...
              libraryName: s.title,
              mediaType:
                s.type === "movie" ? ("movie" as const) : ("tv" as const),
            }));
        }
        const session = await ensureSession(server);
        if (!session) return [];
...
export async function loadHomeServerLibraryPage(
  ...
  const effectiveFilter =
    filter === "all"
      ? homeServerLibraryMediaFilter(options.libraryMediaType)
      : filter;
  try {
    if (server.type === "plex") {
      const base = trimUrl(server.url);
      ...
      if (effectiveFilter !== "all")
        params.set("type", effectiveFilter === "movie" ? "1" : "2");
    ...
    const params = new URLSearchParams({
      ...
      Recursive: "true",
      IncludeItemTypes:
        effectiveFilter === "movie"
          ? "Movie"
          : effectiveFilter === "tv"
            ? "Series"
            : "Movie,Series",
      SortBy:
        sort === "release-newest" || sort === "release-oldest"
          ? "PremiereDate"
          : sort === "title"
            ? "SortName"
            : sort === "rating"
              ...
              : "DateCreated",
      SortOrder:
        sort === "title" || sort === "release-oldest"
          ? "Ascending"
          : "Descending",
      StartIndex: String(offset),
      Limit: String(limit),
      Fields:
        "Overview,PrimaryImageAspectRatio,BasicSyncInfo,ImageTags,BackdropImageTags,ProductionYear,CommunityRating,ProviderIds,DateCreated",
    ...
    const items = (payload.Items ?? [])
      .map((item) => mapItem(base, session.token, item, server))
      .filter((m) => Boolean(m && m.title));
