...
function pickSubtitle(
  ...
  const byLang = (prefix: string) =>
    subtitles.find(
      (s) =>
        (s.lang ?? "").toLowerCase().startsWith(prefix) &&
        !s.isForced &&
        !s.isEmbedded,
    );
  if (pref && pref !== "off") {
...
function isTorrentioResolve(url?: string | null) {
  return Boolean(
    url &&
    /torrentio\.strem\.fun\/(?:resolve\/)?(?:torbox|realdebrid)/i.test(url),
  );
}
...
export function externalPlayerUrl(
  ...
  const containerText =
    `${targetUrl} ${stream.originalUrl ?? ""} ${stream.source ?? ""} ${stream.description ?? ""}`.toLowerCase();
  const ext = /\.m3u8|mpegurl/.test(containerText)
    ? "m3u8"
    : /\.mp4(?:[?#/]|$)/.test(containerText)
      ? "mp4"
      : /\.(mkv|matroska)|remux|\bweb-?dl\b|bluray|\bhevc\b|x265/.test(
            containerText,
          )
        ? "mkv"
        : "mp4";
  ...
  const sub = subtitleUrl(
    pickSubtitle(stream.subtitles, preferredSubtitleLang),
  );
...
function androidIntentUrl(
  ...
  const mime = /\.m3u8|mpegurl/i.test(url)
...
export function externalLaunchMode(
  player: ExternalPlayer,
): "scheme" | "playlist" {
  if (player === "vlc" && isDesktop()) {
...
export function downloadToVlc(
  stream: StreamSource,
  title: string,
  preferredSubtitleLang = "",
) {
  const href = externalPlayerUrl(
    "vlc",
    stream,
    title,
    preferredSubtitleLang,
...
export function openInAnyPlayer(
  stream: StreamSource,
  title: string,
  preferredSubtitleLang = "",
) {
  ...
  if (isAndroid()) {
    const ext = /\.m3u8|mpegurl/i.test(targetUrl)
      ? "m3u8"
      : /\.mp4(?:[?#/]|$)/i.test(targetUrl)
        ? "mp4"
        : "mkv";
    const filename = `${cleanFilename(title)}.${ext}`;
    const sub = subtitleUrl(
...
export async function copyStreamUrl(stream: StreamSource) {
  ...
  if (!targetUrl || typeof navigator === "undefined" || !navigator.clipboard)
    return false;
...
export function triggerDownload(href: string, filename: string) {
  ...
  try {
    const a = document.createElement("a");
    a.href = href;
    a.download = filename || "streamnet-download";
    a.rel = "noopener";
    a.style.position = "fixed";
    a.style.left = "-9999px";
    document.body.appendChild(a);
