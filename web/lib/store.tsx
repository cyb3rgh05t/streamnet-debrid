...
import {
  ...
  installAddon as installAddonManifest,
  loadLocalAddons,
  normalizeAddons,
  saveLocalAddons,
} from "./addons";
...
import {
  ...
  loadIptvChannelIdentities,
  loadIptvGuideForChannels,
  loadIptvSnapshot,
  loadPlaylists,
  savePlaylists,
} from "./iptv";
...
import {
  continueWatchingActivitySignature,
  createTraktActivityCheck,
  type TraktActivitySnapshot,
} from "./traktActivity";
...
function localProfilesMatchAccount(): boolean {
  const owner = (loadStored<string | null>(PROFILES_OWNER_KEY, null) ?? "")
    .trim()
    .toLowerCase();
...
export function getPriorityConfig(
  ...
  return {
    movieProviders:
      (settings.metadataMovieProviders as MetadataProviderId[]) ?? ["tmdb"],
    tvProviders: (settings.metadataTvProviders as MetadataProviderId[]) ?? [
      "tvdb",
      "tmdb",
    ],
    animeProviders:
      (settings.metadataAnimeProviders as MetadataProviderId[]) ?? [
        "anilist",
        "tvdb",
        "tmdb",
      ],
...
function readCachedList(key: string): MediaItem[] {
  const cached = loadStored<{ at: number; items: MediaItem[] } | null>(
    key,
    null,
  );
  return cached?.items?.length && Date.now() - cached.at < LIST_CACHE_TTL_MS
    ? cached.items
    : [];
}
...
function saveCachedList(key: string, items: MediaItem[], max: number) {
  try {
    saveStored(key, {
      at: Date.now(),
      items: items.slice(0, max).map(slimCacheItem),
    });
  } catch {
...
function randomProfileColor() {
  const colors = [
    0xffe50914, 0xff1db954, 0xff3b82f6, 0xfff59e0b, 0xff8b5cf6, 0xffec4899,
...
function makeProfile(name: string, avatarColor: number, avatarId = 0): Profile {
  ...
  return {
    id:
      ...
      `p_${now}_${Math.floor(Math.random() * 1e6)}`,
    name,
    avatarColor,
    avatarId,
...
function filterWatchedContinueWatching(
  items: MediaItem[],
...
async function loadTraktUpNext(
  watchedShowsRows: unknown[],
  includeSpecials: boolean,
  hiddenShowIds: Set<number>,
  isCurrent = () => true,
) {
  ...
  const watchedShows = watchedShowsRows
    .filter((raw) => {
      const traktId = (raw as { show?: { ids?: { trakt?: number } } }).show?.ids
        ?.trakt;
  ...
  const workers = Array.from(
    { length: Math.min(8, watchedShows.length) },
    async () => {
      while (cursor < watchedShows.length && isCurrent()) {
        ...
        const progress = await traktClient
          .showProgress(
            traktId,
            includeSpecials,
            traktProgressActivityKey(watched),
          )
          .catch(() => null);
...
async function hydrateContinueWatchingItems(items: MediaItem[]) {
  ...
  let cursor = 0;
  const workers = Array.from(
    { length: Math.min(10, source.length) },
    async () => {
  ...
  return [...hydrated, ...tail].filter((item): item is MediaItem =>
    Boolean(item),
  );
}
...
function nextLocalEpisode(item: MediaItem): MediaItem | null {
  ...
  if (!current?.episodeCount || episodeNumber <= current.episodeCount)
    return item;
  const next = seasons
    .filter(
      (season) =>
  ...
  return next
    ? {
        ...
        episodeTitle: null,
        episodeStill: null,
        subtitle: `S${next.seasonNumber} E1`,
      }
    : null;
}
...
export interface AppStore {
  ...
  updateTrackingPreferences: (
    patch: Partial<TrackingPreferences>,
  ) => Promise<void>;
...
export function AppProvider({
  ...
  const cwSourceRef = useRef<"none" | "seed" | "fast" | "fresh">(
    initialCw.length ? "seed" : "none",
  );
    ...
    return {
      ...
      uiLanguage:
        ...
        stored.uiLanguage === "en"
          ? "en"
          : "de",
      language: stored.uiLanguage === "en" ? "en-US" : "de-DE",
  ...
  const [traktConnected, setTraktConnected] = useState(
    () => traktClient.isConnected,
  );
  const [mdblistConnected, setMdblistConnected] = useState(
    () => mdblistClient.isConnected,
  );
  const [simklConnected, setSimklConnected] = useState(
  ...
  const stopOwnedPlayback = useCallback(() => {
    ...
    if (owned)
      void reportHomeServerPlayback(owned.stream, owned.settings, "stop").catch(
        () => undefined,
      );
  }, []);
  const iptvRefresh = useRef<{ key: string; promise: Promise<void> } | null>(
    null,
  );
  ...
  const [activeProfileId, setActiveProfileId] = useState<string | null>(() =>
    localProfilesMatchAccount()
      ? loadStored<string | null>(ACTIVE_PROFILE_KEY, null)
      : null,
  );
  ...
  useEffect(
    ...
    [activeProfileId, auth?.userId, stopOwnedPlayback],
  );
  const [trackingPreferences, setTrackingPreferences] =
    useState<TrackingPreferences>(() =>
  ...
  const [avatarImages, setAvatarImagesState] = useState<Record<string, string>>(
    () => loadStored<Record<string, string>>(AVATAR_IMAGES_KEY, {}),
  );
  ...
  const [view, setView] = useState<AppView>(() => {
    ...
    if (
      skip &&
      activeId &&
      stored.some((p) => p.id === activeId && !p.isLocked)
    )
      return "app";
    return "profiles";
  });
  ...
  const hero =
    heroPreview ?? continueWatching[0] ?? categories[0]?.items[0] ?? null;
  ...
  useEffect(() => {
    const effectiveCatalogs = mergeCatalogs(
      settings.catalogs,
  ...
  const persistAddons = useCallback(
    async (next: InstalledAddon[], options: { removedIds?: string[] } = {}) => {
      ...
      await saveCloudAddons(authClient, normalized, activeProfileId, {
  ...
  const refreshData = useCallback(
    (profileIdOverride?: string | null, background = false) => {
      ...
      const isCurrent = () =>
        ...
        authClient.session?.userId === accountId;
      ...
      const activityRead = readsFrom("continueWatching", "trakt")
        ? traktClient.continueWatchingActivity().catch(() => null)
        : Promise.resolve(null);
      const run = (async () => {
        ...
        try {
          ...
          if (cachedCw.length && cwSourceRef.current === "none") {
            ...
            setCategories((current) =>
              current.some((c) => c.id === "continue_watching")
                ? current
                : [
                    {
                      id: "continue_watching",
          ...
          if (cachedWatchlist.length) {
            setWatchlist((current) =>
              current.length ? current : cachedWatchlist,
            );
          }
        } catch {
        ...
        try {
          ...
          if (authClient.session && profileId) {
            ...
            if (cloudTracking) {
              if (!cloudTracking.hasCloudState) {
                ...
                const localTracking =
                  traktClient.token || simklClient.token || mdblistClient.key
                    ? {
                        provider: traktClient.token
                          ? ("TRAKT" as const)
                          : simklClient.token
                            ? ("SIMKL" as const)
                            : ("MDBLIST" as const),
                    ...
                    : null;
                if (localTracking) {
                  await saveCloudTrackingSelection(
                    authClient,
                    profileId,
                    localTracking,
                  ).catch(() => undefined);
                ...
                const preferences =
                  cloudTracking.trackingPreferences ??
                  defaultTrackingPreferences();
          ...
          const settingsChangedDuringPull =
            settingsRef.current !== currentSettings ||
            hasPendingSettings(authClient, profileId);
          if (cloud?.settings && !settingsChangedDuringPull) {
            effectiveSettings = {
              ...defaultSettings,
              ...currentSettings,
              ...cloud.settings,
              catalogs: mergeCatalogs(
                cloud.settings?.catalogs ?? currentSettings.catalogs,
                cloud.settings?.hiddenCatalogIds ??
                  currentSettings.hiddenCatalogIds,
              ),
              iptvPlaylists:
                cloud.settings?.iptvPlaylists ?? currentSettings.iptvPlaylists,
              favoriteChannelIds:
                cloud.settings?.favoriteChannelIds ??
                currentSettings.favoriteChannelIds,
              iptvTvSession:
                cloud.settings?.iptvTvSession ?? currentSettings.iptvTvSession,
              favoriteGroupIds:
                cloud.settings?.favoriteGroupIds ??
                currentSettings.favoriteGroupIds,
              hiddenGroupIds:
                cloud.settings?.hiddenGroupIds ??
                currentSettings.hiddenGroupIds,
              lockedIptvGroupIds:
                cloud.settings?.lockedIptvGroupIds ??
                currentSettings.lockedIptvGroupIds,
              groupOrder:
                cloud.settings?.groupOrder ?? currentSettings.groupOrder,
            };
            if (!sameSettings(settingsRef.current, effectiveSettings))
          ...
          if (cloudReturnedEmpty && source.length && authClient.session) {
            void saveCloudAddons(authClient, source, profileId).catch(
              () => undefined,
            );
          }

          const effectiveCatalogs = mergeCatalogs(
            effectiveSettings.catalogs,
            effectiveSettings.hiddenCatalogIds,
          );
          setCatalogConfigs(
            effectiveCatalogs.filter(
              (catalog) =>
                catalog.enabled && catalog.sourceType !== "home-server",
            ),
          );

          const client = syncClient();
          const traktReady = client.isConnected;
          const activitySignature = continueWatchingActivitySignature(
            await activityRead,
          );
          if (!isCurrent()) return;
          const readFailures = new Set<string>();
          const failedRead = (feature: string) => {
            readFailures.add(feature);
            return [];
          };
          const watchedMoviesRead = traktReady
            ? client.watched("movies").catch(() => failedRead("watched-movies"))
            : Promise.resolve([]);
          const watchedShowsRead = traktReady
            ? client.watched("shows").catch(() => failedRead("watched-shows"))
            : Promise.resolve([]);
          const cwUsesTrakt = readsFrom("continueWatching", "trakt");
          const cwOnlyTrakt =
            cwUsesTrakt &&
            !readsFrom("continueWatching", "simkl") &&
            !readsFrom("continueWatching", "mdblist");
          const sameHistorySources = sameTrackingSources(
            "watched",
            "continueWatching",
          );
          ...
          const readCwHistory = (
            ...
            const read = sameHistorySources
              ? shared.then((rows) => {
                  if (readFailures.has(`watched-${type}`))
                    readFailures.add(`cw-${type}`);
                  return rows;
                })
              : client
                  .watched(type, "continueWatching")
                  .catch(() => failedRead(`cw-${type}`));
            return read.then((rows) => {
              if (!isCurrent()) return rows;
              const completions = completionTimes(
                type === "movies" ? rows : [],
                type === "shows" ? rows : [],
              );
              if (!completions.size) return rows;
              setContinueWatching((current) =>
                pruneCompletedResume(current, completions),
              );
              setCategories((current) =>
                current.flatMap((category) => {
                  if (category.id !== "continue_watching") return [category];
                  const items = pruneCompletedResume(
                    category.items,
                    completions,
                  );
                  return items.length
          ...
          const cwMoviesRead = traktReady
            ? readCwHistory("movies", watchedMoviesRead)
            : Promise.resolve([]);
          const cwShowsRead = traktReady
            ? readCwHistory("shows", watchedShowsRead)
            : Promise.resolve([]);
          // Per-show progress specifically needs Trakt history, even when both trackers supply CW.
          const cwTraktShowsRead = !cwUsesTrakt
            ? Promise.resolve([])
            : cwOnlyTrakt
              ? cwShowsRead
              : traktClient
                  .watched("shows")
                  .catch(() => failedRead("cw-watched"));
          const watchlistReady = ["trakt", "simkl", "mdblist"].some(
            (provider) =>
              readsFrom("watchlist", provider as "trakt" | "simkl" | "mdblist"),
          );
          const [
            historyRows,
            traktRows,
            ...
            traktReady
              ...
              : Promise.resolve([]),
            traktReady
              ? client.playback().catch(() => failedRead("playback"))
              : Promise.resolve([]),
            watchedMoviesRead,
            watchedShowsRead,
            authClient.session
          ...
          const isHiddenShow = (item: MediaItem) =>
            item.mediaType === "tv" &&
            typeof item.traktId === "number" &&
            hiddenShowIds.has(item.traktId);
          const isDismissed = (item: MediaItem) => {
            ...
            const dismissedAt = Math.max(
              cloudDismissals.get(showKey) ?? 0,
              cloudDismissals.get(exactKey) ?? 0,
            );
            ...
            void hydrateTraktItems(fastWatchlistSource)
              .then((hydrated) => {
                if (hydrated.length && isCurrent()) {
                  setWatchlist((current) =>
                    current.length ? current : hydrated,
                  );
          ...
          const cwSorted = dedupeMedia(cwBase)
          ...
          const traktOutage =
            readFailures.has("playback") ||
            readFailures.has("cw-watched") ||
            readFailures.has("cw-movies") ||
            readFailures.has("cw-shows");
          if (
            !traktOutage &&
            ...
            traktActivityRef.current = {
              key: `${accountId ?? "local"}:${profileId}`,
          ...
          if (!traktOutage && isCurrent()) {
            ...
            if (cw.length) {
              ...
              saveCachedList(
                cwCacheKey,
                reconcile(readCachedList(cwCacheKey)),
                20,
              );
              setCategories((current) => {
                const others = current.filter(
                  (c) => c.id !== "continue_watching",
                );
                const previous =
                  current.find((c) => c.id === "continue_watching")?.items ??
                  [];
                return [
                  {
                    id: "continue_watching",
                    title: "Continue Watching",
                    items: reconcile(previous),
                  },
                  ...others,
                ];
              });
              ...
              setContinueWatching([]);
              setCategories((current) =>
                current.filter((c) => c.id !== "continue_watching"),
              );
  ...
  const refreshIptv = useCallback(async () => {
    ...
    const isCurrent = () =>
      activeProfileIdRef.current === profileId &&
      authClient.session?.userId === account &&
      iptvPlaylistSignature(settingsRef.current.iptvPlaylists) === signature;
    const run = (async () => {
      setBusy("Loading TV");
      try {
        ...
        if (isCurrent())
          setIptvSnapshot({
            ...loadedIptv,
            signature,
            scopeKey: `${account ?? "local"}:${profileId ?? "local"}`,
          });
      } catch (error) {
        if (isCurrent())
          setToast(
            error instanceof Error ? error.message : "Failed to load Live TV",
          );
      } finally {
    ...
    return run.finally(() => {
      if (iptvRefresh.current?.promise === run) iptvRefresh.current = null;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    if (
      ...
      iptvSnapshot.signature !== iptvPlaylistSignature(settings.iptvPlaylists)
    )
      return;
    let cancelled = false;
    const profileId = activeProfileId;
    void loadIptvChannelIdentities(settings.iptvPlaylists, channels, {
      ...
      setIptvSnapshot((current) =>
        current.scopeKey !== iptvSnapshot.scopeKey ||
        current.signature !== iptvSnapshot.signature ||
        (current.allChannels ?? current.channels) !== channels
          ? current
          : {
              ...current,
              ...
              channels: current.channels.map(
                (channel) => byId.get(channel.id) ?? channel,
              ),
            },
      );
    });
    ...
    iptvSnapshot.allChannels,
    iptvSnapshot.channels,
    iptvSnapshot.signature,
    iptvSnapshot.identitiesLoaded,
  ...
  const loadIptvGuide = useCallback(
    async (channels: IptvChannel[]) => {
      ...
      const scopedKey = (id: string) =>
        `${account}:${profileId}:${signature}:${id}`;
      ...
      const missing = channels.filter(
        (channel) =>
          !isFresh(channel.id) &&
          !iptvGuideInFlightRef.current.has(scopedKey(channel.id)) &&
          (guideRetryAfter.current.get(scopedKey(channel.id)) ?? 0) <
            Date.now(),
      );
      if (!missing.length) return;
      missing.forEach((channel) => {
        iptvGuideInFlightRef.current.add(scopedKey(channel.id));
        guideRetryAfter.current.set(scopedKey(channel.id), Date.now() + 60_000);
      });
      if (guideRetryAfter.current.size > 2000)
        for (const [key, expiry] of guideRetryAfter.current)
          if (expiry < Date.now()) guideRetryAfter.current.delete(key);
      try {
        const guide = await loadIptvGuideForChannels(
          currentSettings.iptvPlaylists,
          missing,
        );
        if (
        ...
        missing.forEach((channel) =>
          iptvGuideInFlightRef.current.delete(scopedKey(channel.id)),
        );
      }
    },
  ...
  useEffect(() => {
    if (
      view !== "app" ||
      !traktConnected ||
      (authClient.session && !cloudProfilesHydrated)
    )
      return;
    const monitor = createTraktActivityCheck({
      ...
      read: async () =>
        continueWatchingActivitySignature(
          await traktClient.continueWatchingActivity(),
        ),
  ...
  useEffect(() => {
    if (view === "login" || (authClient.session && !cloudProfilesHydrated))
      return undefined;
    let lastRefreshAt = 0;
  ...
  useEffect(() => {
    ...
    if (!authClient.session || !activeProfileId) {
      setSettingsSyncState("local");
      return;
    }
    if (authClient.session && !cloudProfilesHydrated) return;
    ...
    if (
      ...
      snapshot === lastSyncedSettingsRef.current
    ) {
    ...
    try {
      if (lastSyncedSettingsRef.current) {
        const parsed = JSON.parse(lastSyncedSettingsRef.current) as {
          settings?: AppSettings;
          activeProfileId?: string | null;
        };
        if (parsed.activeProfileId === activeProfileId && parsed.settings)
    ...
    try {
      queueSettings(authClient, activeProfileId, settings, baseline);
    } catch (error) {
      setSettingsSyncState("error");
      setToast(
    ...
    void pullCloudProfiles(authClient)
      .then((cloud) => {
        ...
        if (cloud.profiles.length) {
          ...
          const selectedId = hydratedProfileId(
  ...
  const updateSettings = useCallback((patch: Partial<AppSettings>) => {
    setSettings((prev) => {
      ...
      if (patch.uiLanguage) {
        next.language = patch.uiLanguage === "en" ? "en-US" : "de-DE";
        (
          next as AppSettings & { userSelectedLanguage?: boolean }
  ...
  useEffect(() => {
    setTmdbLanguage(settings.uiLanguage === "en" ? "en-US" : "de-DE");
  }, [settings.uiLanguage]);
  ...
  const markWatchedLocally = useCallback(
    (
      item: {
        mediaType: MediaItem["mediaType"];
        id: number;
      ...
      if (watched) {
        setContinueWatching((prev) =>
          prev.filter(
            (entry) =>
              mediaWatchKey(entry) !== key &&
              mediaWatchKey(entry) !== `${item.mediaType}:${item.id}`,
          ),
        );
        setCategories((prev) =>
          prev
            .map((cat) =>
              cat.id === "continue_watching"
                ? {
                    ...cat,
            ...
            .filter(
              (cat) => cat.id !== "continue_watching" || cat.items.length,
            ),
        );
      }
    },
  ...
  const mergeStreams = useCallback((incoming: StreamSource[]) => {
    setStreams((prev) => {
      ...
      return [
        ...incoming,
        ...extras.filter((s) => !seen.has(s.url ?? s.source)),
      ];
    });
  }, []);
  ...
  const appendVodSources = useCallback(
    (item: MediaItem, season?: number, episode?: number) => {
      ...
      return (async () => {
        try {
          const { findMovieVodSources, findEpisodeVodSource } =
            await import("./xtreamVod");
          ...
          if (!sources.length || sourceGeneration.current !== generation)
            return [];
          setStreams((prev) => {
  ...
  const appendHomeServerSources = useCallback(
    (item: MediaItem, season?: number, episode?: number) => {
      const generation = sourceGeneration.current;
      ...
      return (async () => {
        try {
          const {
            resolveHomeServerMovieSources,
            resolveHomeServerEpisodeSources,
          } = await import("./homeserver");
          const target = {
            title: item.title,
            ...
            tmdbId:
              item.tmdbId ??
              (item.id > 0 && !item.isHomeServer ? item.id : undefined),
          };
          const sources =
            season && episode
              ? await resolveHomeServerEpisodeSources(
                  servers,
                  target,
                  season,
                  episode,
  ...
  const openDetails = useCallback(async (item: MediaItem) => {
    const generation = ++sourceGeneration.current;
    const publish = (rows: StreamSource[]) => {
      if (sourceGeneration.current === generation) mergeStreams(rows);
    };
    setSelected(item);
    ...
    if (!resolvedId && !item.isHomeServer) {
      setBusy("");
      setToast(
        localize(
          settingsRef.current.uiLanguage,
          "Für diesen Titel konnten keine Metadaten gefunden werden.",
          "Metadata could not be matched for this title.",
        ),
      );
      return;
    }
    const detailsTarget =
      item.isHomeServer && item.tmdbId
        ? { ...item, id: item.tmdbId }
        : { ...item, id: resolvedId ?? item.id };
    const detailed = await getDetails(detailsTarget, priorityConfig).catch(
      () => item,
    );
    if (sourceGeneration.current !== generation) return;
    ...
    if (item.mediaType === "movie") {
      ...
      setSelectedEpisode({
        season: withResumeEpisode.seasonNumber,
        episode: withResumeEpisode.episodeNumber,
      });
      setBusy("Finding sources");
      appendVodSources(
        withResumeEpisode,
  ...
  const loadEpisodeStreams = useCallback(
    async (item: MediaItem, season: number, episode: number) => {
      const generation = ++sourceGeneration.current;
      const publish = (rows: StreamSource[]) => {
        if (sourceGeneration.current === generation) mergeStreams(rows);
      };
  ...
  const playStream = useCallback(
    (
      ...
      if (
        ...
        stream = {
          ...stream,
          resumePositionSeconds: selected.resumePositionSeconds,
        };
      }
      ...
      const isCurrent = () =>
        ...
        accountId === authClient.session?.userId;
      if (!stream.url) {
        setToast(
          localize(
            settingsRef.current.uiLanguage,
            "Diese Quelle hat noch keine direkte Stream-URL.",
            "This source has no direct stream URL yet.",
          ),
        );
        return;
      }
      ...
      if (
        ...
        const externalTitle =
          selected?.title ?? stream.source ?? "StreamNet stream";
        ...
        const cached = parseDebridStream(stream.url)
          ? cachedDebridDirectUrl(stream.url)
          : null;
        const target = cached
          ? { ...stream, url: cached, originalUrl: stream.url }
          : stream;
        setToast(
          preferredPlayer === "vlc" && externalLaunchMode("vlc") === "playlist"
            ? localize(
                settingsRef.current.uiLanguage,
                "VLC-Wiedergabeliste gespeichert. Öffne sie in deinen Downloads.",
                "VLC playlist saved; open it from your downloads to play.",
              )
            : `${localize(settingsRef.current.uiLanguage, "Wird geöffnet in", "Opening in")} ${preferredPlayer.toUpperCase()}...`,
        );
      ...
      const timeout = window.setTimeout(() => {
        ...
        if (isCurrent())
          setToast(
            localize(
              settingsRef.current.uiLanguage,
              "Die Quelle hat nicht geantwortet. Bitte versuche es erneut oder wähle eine andere Quelle.",
          ...
          if (isCurrent() && !controller.signal.aborted)
            setToast(
              error instanceof Error
                ...
                : localize(
                    settingsRef.current.uiLanguage,
                    "Diese Quelle konnte nicht vorbereitet werden.",
                    "Could not prepare this source.",
                  ),
            );
        })
  ...
  const advanceEpisode = useCallback(async (): Promise<boolean> => {
    if (!selected || selected.mediaType !== "tv" || !selectedEpisode)
      return false;
    const generation = ++sourceGeneration.current;
    const playback = ++playbackGeneration.current;
    const isCurrent = () =>
      sourceGeneration.current === generation &&
      playbackGeneration.current === playback;
    try {
      const next = nextLocalEpisode({
        ...selected,
        timeRemainingLabel: "Up next",
        seasonNumber: selectedEpisode.season,
        episodeNumber: selectedEpisode.episode + 1,
      });
      if (!next?.seasonNumber || !next.episodeNumber) {
        setToast(
          localize(
            settingsRef.current.uiLanguage,
            "Du hast die letzte verfügbare Folge erreicht.",
            "You have reached the last available episode.",
          ),
        );
        return false;
      }
      const episodes = await getSeasonEpisodes(
        selected.tmdbId ?? selected.id,
        next.seasonNumber,
      );
      if (!isCurrent()) return false;
      const episode = episodes.find(
        (item) => item.episodeNumber === next.episodeNumber,
      );
      if (
        !episode ||
        (episode.airDate && Date.parse(episode.airDate) > Date.now())
      ) {
        setToast(
          localize(
            settingsRef.current.uiLanguage,
            "Die nächste Folge ist noch nicht verfügbar.",
      ...
      const choose = (rows: StreamSource[]) => {
        ...
        const candidate = rows.find(
          (row) => row.url && playbackPlan(row).route === "here",
        );
        if (!candidate) return;
        started = true;
        setSelectedEpisode({
        ...
        setActiveStream(null);
        playStream(
          { ...candidate, autoSelect: true, resumePositionSeconds: 0 },
          { forceBrowser: true },
        );
      };
      setStreams([]);
      ...
      await Promise.allSettled([
        ...
        appendVodSources(selected, next.seasonNumber, next.episodeNumber).then(
          choose,
        ),
      ]);
      if (!started && isCurrent())
        setToast(
          localize(
            settingsRef.current.uiLanguage,
            "Für die nächste Folge wurde keine im Browser abspielbare Quelle gefunden. Wähle eine Quelle manuell.",
            "No browser-playable source for the next episode. Choose a source manually.",
          ),
        );
      return started;
    } catch {
      if (isCurrent())
        setToast(
          localize(
            settingsRef.current.uiLanguage,
            "Die nächste Folge konnte nicht geladen werden. Bitte versuche es erneut.",
            "Could not load the next episode. Please retry.",
          ),
        );
      return false;
    }
  ...
  const playTrailer = useCallback(
    async (item: MediaItem) => {
      ...
      if (!url) {
        const priorityConfig = getPriorityConfig(settingsRef.current);
        const detailed = await getDetails(item, priorityConfig).catch(
          () => item,
        );
      ...
      if (!url) {
        setToast(
          localize(
            settingsRef.current.uiLanguage,
            "Für diesen Titel ist kein Trailer verfügbar.",
  ...
  const openLiveExternally = useCallback(
    (stream: StreamSource, title: string): boolean => {
      ...
      openExternalPlayer(
        ...
        stream,
        title,
        settingsRef.current.defaultSubtitle,
      );
      return true;
    },
    [],
  ...
  const recordChannelPlayback = useCallback((channel: IptvChannel) => {
    setSettings((current) => ({
      ...current,
      iptvTvSession: recordTvPlayback(current.iptvTvSession, channel),
    }));
  }, []);
  ...
  const playCatchup = useCallback(
    (channel: IptvChannel, program: IptvProgram) => {
      playbackPreparation.current?.abort();
      stopOwnedPlayback();
      playbackGeneration.current++;
      const url = buildXtreamCatchupUrl(
        settingsRef.current.iptvPlaylists,
        channel,
        program,
      );
      if (!url) {
        setToast(
          localize(
            settingsRef.current.uiLanguage,
            "Für diesen Sender ist keine zeitversetzte Wiedergabe verfügbar.",
            "Catch-up is not available for this channel.",
          ),
        );
        return;
      }
  ...
  const loadCatalogRow = useCallback(
    (catalog: CatalogConfig) =>
      loadCatalog(
        catalog,
        settings.language,
        addonsRef.current,
        settingsRef.current.homeServers,
      ),
    [settings.language],
  );
  ...
  const installAddon = useCallback(
    async (url: string) => {
      ...
      const next = [
        addon,
        ...addonsRef.current.filter((candidate) => candidate.id !== addon.id),
      ];
  ...
  const removeAddon = useCallback(
    async (addon: InstalledAddon) => {
      const next = addonsRef.current.filter(
  ...
  const setAddonsState = useCallback(
    async (next: InstalledAddon[]) => {
      await persistAddons(next);
      setSettings((prev) => ({
        ...prev,
        disabledAddonIds: next
  ...
  const signIn = useCallback(
    async (email: string, password: string, mode: "sign-in" | "sign-up") => {
      ...
      try {
        ...
        const session =
          mode === "sign-up"
            ? await authClient.signUp(trimmedEmail, password)
            : await authClient.signIn(trimmedEmail, password);
  ...
  const pollTrakt = useCallback(async () => {
    ...
    const nextPreferences: TrackingPreferences = {
      ...
      continueWatchingReadMode: upgradeMode(
        previousPreferences.continueWatchingReadMode,
      ),
    ...
    if (targetProfileId)
      saveTrackingPreferences(targetProfileId, nextPreferences);
  ...
  const disconnectTrakt = useCallback(async () => {
    ...
    const fallback = simklClient.isConnected
      ? ("simkl" as const)
      : mdblistClient.isConnected
        ? ("mdblist" as const)
        : ("auto" as const);
    ...
    if (targetProfileId) {
      await saveCloudTrackingSelection(authClient, targetProfileId, {
        provider: simklClient.isConnected
          ? "SIMKL"
          : mdblistClient.isConnected
            ? "MDBLIST"
            : "NONE",
  ...
  const connectMdblist = useCallback(
    async (key: string) => {
      const targetProfileId = activeProfileIdRef.current;
      ...
      const hasPrimaryTracker =
        traktClient.isConnected || simklClient.isConnected;
      const mdbPreferences: TrackingPreferences = {
        watchlistReadMode: hasPrimaryTracker
          ? currentPreferences.watchlistReadMode
          : "mdblist",
        continueWatchingReadMode: hasPrimaryTracker
          ? currentPreferences.continueWatchingReadMode
          : "mdblist",
        watchedReadMode: hasPrimaryTracker
          ? currentPreferences.watchedReadMode
          : "mdblist",
        writeToTrakt:
          traktClient.isConnected && currentPreferences.writeToTrakt,
        writeToSimkl:
          simklClient.isConnected && currentPreferences.writeToSimkl,
      };
      if (targetProfileId)
        saveTrackingPreferences(targetProfileId, mdbPreferences);
  ...
  const disconnectMdblist = useCallback(async () => {
    ...
    const fallback = traktClient.isConnected
      ...
      : simklClient.isConnected
        ? ("simkl" as const)
        : ("auto" as const);
    const replaceMdbList = (mode: TrackingPreferences["watchlistReadMode"]) =>
      mode === "mdblist" ? fallback : mode;
    const nextPreferences: TrackingPreferences = {
      watchlistReadMode: replaceMdbList(current.watchlistReadMode),
      continueWatchingReadMode: replaceMdbList(
        current.continueWatchingReadMode,
      ),
  ...
  const pollSimkl = useCallback(async () => {
    ...
    if (!token) {
      throw new Error(
        "Simkl has not approved this PIN yet. Please approve the code on Simkl.",
      );
    }
    if (activeProfileIdRef.current !== targetProfileId) return;
    traktClient.setProfile(targetProfileId);
    ...
    const previousPreferences = loadTrackingPreferences(targetProfileId);
    const mode = traktClient.isConnected
      ? ("trakt" as const)
      : ("simkl" as const);
    const upgradeMode = (current: TrackingPreferences["watchlistReadMode"]) =>
  ...
  const updateTrackingPreferences = useCallback(
    async (patch: Partial<TrackingPreferences>) => {
      const profileId = activeProfileIdRef.current;
      if (!profileId) return;
  ...
  const loadTraktLists = useCallback(async (): Promise<
  ...
  const fetchTraktListItems = useCallback(
    async (source: string, cacheKey: string): Promise<MediaItem[]> => {
      let rows: unknown[] = [];
  ...
  const loadTraktListItems = useCallback(
    async (source: string): Promise<MediaItem[]> => {
      if (!traktClient.isConnected) return [];
  ...
  const loadTrackerLibrary = useCallback(
    async (
      provider: "trakt" | "simkl",
      source: string,
    ): Promise<MediaItem[]> => {
      if (provider === "trakt") {
        if (!traktClient.isConnected)
          throw new Error("Connect Trakt in Settings.");
        if (source === "watched") {
          const [movies, shows] = await Promise.all([
            traktClient.watched("movies"),
            traktClient.watched("shows"),
          ]);
          return hydrateTraktItems(
            dedupeMedia(
              [...(movies as unknown[]), ...(shows as unknown[])].map(
                traktItemToMedia,
              ),
            ),
          );
        ...
        return fetchTraktListItems(
          source,
          `arvio.web.traktlist.v2:${authClient.session?.userId ?? "local"}:${traktClient.currentProfileId}:${source}`,
        );
      }
      if (!simklClient.isConnected)
        throw new Error("Connect Simkl in Settings.");
      if (
        !["plantowatch", "watching", "completed", "hold", "dropped"].includes(
          source,
        )
      )
        throw new Error("Unknown Simkl library");
      const rows = await simklClient.library(
        source as "plantowatch" | "watching" | "completed" | "hold" | "dropped",
      );
  ...
  const persistProfiles = useCallback(
    (next: Profile[], activeId: string | null) => {
  ...
  const selectProfile = useCallback(
    async (profile: Profile, pin?: string) => {
      if (
        profile.isLocked &&
        profile.pin &&
        !(await verifyProfilePin(pin ?? "", profile.pin))
      ) {
        setToast(
          localize(
            settingsRef.current.uiLanguage,
            "Entsperre dieses Profil mit seiner PIN.",
            "Unlock this profile with its PIN.",
          ),
        );
        return;
      }
      const updated = profiles.map((p) =>
        p.id === profile.id ? { ...p, lastUsedAt: Date.now() } : p,
      );
  ...
  const createProfile = useCallback(
    async (name: string, avatarColor: number, avatarId: number) => {
    ...
    async (item: MediaItem) => {
      ...
      setCategories((prev) =>
        prev
          ...
          .filter((cat) => cat.id !== "continue_watching" || cat.items.length),
      );
      ...
      if (authClient.session) {
        try {
          await removeContinueWatchingProgress(
            authClient,
            item,
            activeProfileId,
          );
      ...
      if (activeSyncProvider() !== "none") {
        try {
          await syncClient().dismissFromContinueWatching({
            mediaType: item.mediaType,
            tmdbId: item.id,
  ...
  const value = useMemo<AppStore>(
    () => ({
      ...
      addons,
      addonsReady,
      iptvSnapshot: isCurrentIptvSnapshot(
        iptvSnapshot,
        `${auth?.userId ?? "local"}:${activeProfileId ?? "local"}`,
        iptvPlaylistSignature(settings.iptvPlaylists),
      )
        ? iptvSnapshot
        : emptyIptv,
      query,
