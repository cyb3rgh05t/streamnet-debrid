...
function parsePayload(
  payload: RawPayload | string | null | undefined,
): RawPayload {
  if (!payload) return {};
...
function bumpFieldTs(root: RawPayload, key: string, changedAt = Date.now()) {
  const map = (
    root.fieldUpdatedAt && typeof root.fieldUpdatedAt === "object"
      ? root.fieldUpdatedAt
      : {}
...
function normalizeIptvPlaylist(
  value: unknown,
  fallbackId: string,
): IptvPlaylistEntry | null {
  const raw = objectRecord(value);
  const xtreamTriplet = [
    raw.host ?? raw.server ?? raw.portal ?? raw.baseUrl,
    raw.username ?? raw.user,
    raw.password ?? raw.pass,
  ]
  ...
  const epgUrl = stringValue(
    raw.epgUrl ?? raw.epg_url ?? raw.xmltvUrl ?? raw.xmltv_url,
  );
...
function normalizeIptvPlaylists(
  value: unknown,
  ...
  return values.length
    ? values
        .map((playlist, index) =>
          normalizeIptvPlaylist(playlist, `${fallbackPrefix}-${index + 1}`),
        )
...
function androidSubtitleColor(settings: AppSettings) {
  return (
    settings.subtitleColorName || webSubtitleColorName(settings.subtitleColor)
  );
}

function webSubtitleColorName(
  value: unknown,
): AppSettings["subtitleColorName"] {
  const raw = String(value ?? "")
    .trim()
    .toLowerCase();
  if (raw.includes("yellow") || raw === "#ffff00" || raw === "#ffeb3b")
    return "Yellow";
  if (raw.includes("green") || raw === "#00ff00" || raw === "#4caf50")
    return "Green";
  if (raw.includes("cyan") || raw === "#00ffff" || raw === "#00bcd4")
    return "Cyan";
  if (raw.includes("red") || raw === "#ff0000" || raw === "#f44336")
    return "Red";
  if (raw.includes("orange") || raw === "#ffa500" || raw === "#ff9800")
    return "Orange";
  if (raw.includes("blue") || raw === "#0000ff" || raw === "#2196f3")
    return "Blue";
  if (raw.includes("violet") || raw.includes("purple") || raw === "#8b5cf6")
    return "Violet";
  return "White";
}
...
function webFrameRateMode(
  value: unknown,
): AppSettings["frameRateMatchingMode"] {
  switch (
    String(value ?? "")
      .trim()
      .toLowerCase()
...
function androidProfileSettings(settings: AppSettings) {
  return {
    ...
    volumeBoostDb: Math.max(
      0,
      Math.min(15, Number(settings.volumeBoostDb) || 0),
    ),
    ...
    homeServerConnectionJson: serializeHomeServerConnectionJson(
...
function settingsFromAndroidProfile(value: unknown): Partial<AppSettings> {
  ...
  if ("defaultAudioLanguage" in state)
    partial.audioLanguage = String(state.defaultAudioLanguage || "");
  if ("contentLanguage" in state)
    partial.language = String(state.contentLanguage || "en-US");
  if ("trailerAutoPlay" in state)
    partial.trailerAutoPlay = Boolean(state.trailerAutoPlay);
  if ("trailerSoundEnabled" in state)
    partial.trailerSound = Boolean(state.trailerSoundEnabled);
  if ("trailerDelaySeconds" in state)
    partial.trailerDelaySeconds = Number(state.trailerDelaySeconds) || 2;
  if ("trailerInCards" in state)
    partial.trailerInCards = Boolean(state.trailerInCards);
  ...
  if ("spoilerBlurEnabled" in state)
    partial.spoilerBlur = Boolean(state.spoilerBlurEnabled);
  if ("volumeBoostDb" in state)
    partial.volumeBoostDb = Math.max(
      0,
      Math.min(15, Number(state.volumeBoostDb) || 0),
    );
  if ("includeSpecials" in state)
    partial.includeSpecials = Boolean(state.includeSpecials);
  if ("dnsProvider" in state)
    partial.dnsProvider = normalizedDns(state.dnsProvider);
  if ("subtitleSize" in state)
    partial.subtitleSize = webSubtitleSize(state.subtitleSize);
  ...
  if ("subtitleOffset" in state)
    partial.subtitleOffset = webSubtitleOffset(state.subtitleOffset);
  if ("subtitleStyle" in state)
    partial.subtitleStyle = webSubtitleStyle(state.subtitleStyle);
  ...
  if ("subtitleAiEnabled" in state)
    partial.aiSubtitlesEnabled = Boolean(state.subtitleAiEnabled);
  if ("subtitleAiAutoSelect" in state)
    partial.aiAutoSelect = Boolean(state.subtitleAiAutoSelect);
  if ("subtitleAiApiKey" in state)
    partial.aiApiKey = String(state.subtitleAiApiKey || "");
  if ("subtitleAiModel" in state) {
    const model = String(state.subtitleAiModel || "").toLowerCase();
    partial.aiSubtitleModel =
      model === "gemini" ? "gemini" : model === "off" ? "off" : "groq";
  }
  if ("subtitleStylized" in state)
    partial.subtitleStylized = Boolean(state.subtitleStylized);
  if ("secondarySubtitle" in state)
    partial.secondarySubtitle = String(state.secondarySubtitle || "");
  if ("filterSubtitlesByLanguage" in state)
    partial.filterSubtitlesByLanguage = Boolean(
      state.filterSubtitlesByLanguage,
    );
  if (
  ...
  if ("autoPlayNext" in state)
    partial.autoPlayNext = Boolean(state.autoPlayNext);
  if ("autoPlaySingleSource" in state)
    partial.autoPlaySingleSource = Boolean(state.autoPlaySingleSource);
  if ("autoPlayMinQuality" in state)
    partial.autoPlayMinQuality = webQuality(state.autoPlayMinQuality);
  if ("torrServerBaseUrl" in state)
    partial.torrServerBaseUrl = stringValue(state.torrServerBaseUrl);
...
function iptvFromAndroid(
  ...
  const profilePlaylists = normalizeIptvPlaylists(
    state.playlists,
    "cloud-profile",
  );
  const rootSettingsPlaylists = normalizeIptvPlaylists(
    rootSettings.iptvPlaylists,
    "cloud-settings",
  );
  const rootPlaylists = normalizeIptvPlaylists(
    rootState.iptvPlaylists ?? rootState.playlists,
    "cloud-root",
  );
  const legacyM3uUrl =
    stringValue(state.m3uUrl) || stringValue(rootState.iptvM3uUrl);
  const legacyEpgUrl =
    stringValue(state.epgUrl) || stringValue(rootState.iptvEpgUrl);
  ...
  return {
    ...
    favoriteChannelIds:
      value !== undefined
        ? stringArray(state.favoriteChannels)
        : stringArray(rootState.iptvFavoriteChannels),
    iptvTvSession: normalizeTvSession(
      (state as Record<string, unknown>).tvSession,
    ),
    favoriteGroupIds:
      value !== undefined
        ? stringArray(state.favoriteGroups)
        : stringArray(rootState.iptvFavoriteGroups),
    ...
    iptvSortOrder:
      state.sortOrder === "number" || state.sortOrder === "name"
        ? state.sortOrder
        : "provider",
    iptvStalkerUrl: stringValue(
      state.stalkerPortalUrl ?? rootState.iptvStalkerUrl,
    ),
    iptvStalkerMac: stringValue(
      state.stalkerMacAddress ?? rootState.iptvStalkerMac,
    ),
  };
}

function androidCwToHistory(
  item: AndroidContinueWatchingItem,
  profileId?: string | null,
  ...
  const progress = Math.max(
    0,
    Math.min(0.99, Number(item.progress ?? 0) / 100),
  );
  return {
    user_id: "",
...
function historyToAndroidCw(
  entry: Omit<WatchHistoryEntry, "user_id">,
): AndroidContinueWatchingItem {
  return {
...
function androidContinueWatchingItems(
  ...
  const readByProfile = (key: string) => {
    ...
    if (!Object.keys(byProfile).length)
      return [] as AndroidContinueWatchingItem[];
    return profileId
      ? arrayValue<AndroidContinueWatchingItem>(byProfile[profileId])
      : Object.values(byProfile).flatMap((items) =>
          arrayValue<AndroidContinueWatchingItem>(items),
        );
  };
  const scoped = [
...
export async function pullRawPayload(auth: AuthClient): Promise<RawPayload> {
  ...
  if (
    rawPayloadCache &&
    rawPayloadCache.userId === userId &&
    Date.now() - rawPayloadCache.at < RAW_PAYLOAD_TTL_MS
  ) {
  ...
  const promise = fetchRawPayload(auth)
    .then((payload) => {
      if (
        generation === rawPayloadGeneration &&
...
export async function mutateCloudPayload(
  auth: AuthClient,
  mutator: (root: RawPayload) => void,
) {
  if (!auth.session) return;
  const userId = auth.session.userId;
  ...
  try {
    ...
    if (mutationQueues.get(userId) === task) mutationQueues.delete(userId);
  }
}

export async function pullCloudPayload(
  auth: AuthClient,
  profileId?: string | null,
): Promise<CloudPayload> {
  const root = await pullRawPayload(auth);
  const profileSettings = settingsFromAndroidProfile(
    scopedValue(root, "profileSettingsById", profileId),
  );
  const iptvSettings = iptvFromAndroid(
    scopedValue(root, "iptvByProfile", profileId),
    root,
  );
  const profileCatalogs = scopedValue<AppSettings["catalogs"]>(
    root,
    "catalogsByProfile",
    profileId,
  );
  const hiddenCatalogIds = scopedValue<string[]>(
    root,
    "hiddenPreinstalledByProfile",
    profileId,
  );
  const hiddenHomeServerCatalogIds = scopedValue<string[]>(
    root,
    "hiddenHomeServerByProfile",
    profileId,
  );
  const profileAddons = scopedValue<InstalledAddon[]>(
    root,
    "addonsByProfile",
    profileId,
  );
  const legacySettings = objectRecord<unknown>(
    root.settings,
  ...
  const legacyHiddenCatalogIds = arrayValue<string>(
    root.hiddenPreinstalledCatalogs,
  );
  ...
  if ("customUserAgent" in root)
    globalSettings.customUserAgent = String(root.customUserAgent ?? "");
  if ("skipProfileSelection" in root)
    globalSettings.skipProfileSelection = Boolean(root.skipProfileSelection);
  if ("subtitleAiEnabled" in root)
    globalSettings.aiSubtitlesEnabled = Boolean(root.subtitleAiEnabled);
  if ("subtitleAiAutoSelect" in root)
    globalSettings.aiAutoSelect = Boolean(root.subtitleAiAutoSelect);
  if ("subtitleAiApiKey" in root)
    globalSettings.aiApiKey = String(root.subtitleAiApiKey ?? "");
  ...
  if ("subtitleAiModel" in root) {
    ...
    if (model === "off" || model === "groq" || model === "gemini")
      globalSettings.aiSubtitleModel = model;
  }
  if ("subtitleRemoveHearingImpaired" in root)
    globalSettings.removeHearingImpaired = Boolean(
      root.subtitleRemoveHearingImpaired,
    );
  return {
    ...
    addons: unionAddons(
      arrayValue<InstalledAddon>(profileAddons),
      arrayValue<InstalledAddon>(root.addons),
    ),
    settings: {
      ...legacySettings,
      ...profileSettings,
      ...globalSettings,
      ...iptvSettings,
      ...(arrayValue(profileCatalogs).length
        ? { catalogs: arrayValue(profileCatalogs) as AppSettings["catalogs"] }
        : legacyCatalogs.length
          ? { catalogs: legacyCatalogs }
          : {}),
      ...(hiddenCatalogIds !== undefined
        ? { hiddenCatalogIds: arrayValue<string>(hiddenCatalogIds) }
        : legacyHiddenCatalogIds.length
          ? { hiddenCatalogIds: legacyHiddenCatalogIds }
          : {}),
      ...(hiddenHomeServerCatalogIds !== undefined
        ? {
            hiddenHomeServerCatalogIds: arrayValue<string>(
...
export function mergeIptvSettings(
  ...
  for (const [field, value] of Object.entries(incoming)) {
    ...
    merged[field] =
      base &&
      field in existing &&
      (field === "favoriteChannels" || field === "favoriteGroups")
        ? mergeFavoriteEdits(
            stringArray(existing[field]),
            stringArray(value),
            stringArray(base[field]),
          )
        : value;
  }
  return merged;
...
export async function saveCloudSettings(
  ...
  await mutateCloudPayload(auth, (root) => {
    ...
    void addons;
    const {
      customTmdbApiKey: _k1,
      customTvdbApiKey: _k2,
      customTvdbUserPin: _k3,
      ...sanitizedSettings
    } = settings;
    ...
    const globalFields: Array<[string, unknown, unknown]> = [
      ...
      [
        ...
        settings.removeHearingImpaired,
        baseline?.removeHearingImpaired,
      ],
    ];
    ...
    const iptvExisting = objectRecord(
      scopedValue(root, "iptvByProfile", profileId),
    );
    ...
    if (profileId) {
      ...
      const newProfile = androidProfileSettings(settings) as Record<
        string,
        unknown
      >;
      const baseProfile = baseline
        ? (androidProfileSettings(baseline) as Record<string, unknown>)
        : null;
      const existing = (scopedValue<Record<string, unknown>>(
        root,
        "profileSettingsById",
        profileId,
      ...
      if (
        !baseline ||
        !sameFieldValue(settings.defaultSubtitle, baseline.defaultSubtitle)
      ) {
...
export async function pullCloudProfiles(
  auth: AuthClient,
): Promise<CloudProfiles> {
  const root = await pullRawPayload(auth);
  return {
...
function normalizeCloudTrackingProvider(
  value: unknown,
): CloudTrackingProvider | null {
  const normalized =
...
function normalizeTrackingReadMode(
  value: unknown,
  fallback: TrackingReadMode,
): TrackingReadMode {
  const normalized =
...
function readCloudTraktToken(
  ...
  const expiresAt =
    rawExpiresAt > 0 && rawExpiresAt < 1_000_000_000_000
      ? rawExpiresAt * 1000
      : rawExpiresAt;
...
function readCloudSimklToken(
  root: RawPayload,
  profileId: string,
): SimklToken | null {
  const directTokens = objectRecord<{
    access_token?: string;
    accessToken?: string;
  }>(root.simklTokens);
  ...
  const hasCanonicalCredentialState =
    Number(selection?.simklCredentialUpdatedAt ?? 0) > 0;
...
export async function pullCloudTrackingSelection(
  ...
  const root = await pullRawPayload(auth);
  const selections = objectRecord<Record<string, unknown>>(
    root.mdbListSyncByProfile,
  );
  const selection = selections[profileId] ?? {};
  ...
  const mdbListApiKey =
    typeof rawMdbListKey === "string" ? rawMdbListKey.trim() || null : null;
  ...
  const provider: CloudTrackingProvider =
    requestedProvider === "TRAKT" && traktToken
      ? "TRAKT"
      : requestedProvider === "SIMKL" && simklToken
        ...
        : requestedProvider === "MDBLIST" && mdbListApiKey
          ? "MDBLIST"
          : "NONE";
  const storedProviderCount =
    Number(Boolean(traktToken)) +
    Number(Boolean(simklToken)) +
    Number(Boolean(mdbListApiKey));
  const defaultMode: TrackingReadMode =
    provider === "TRAKT"
      ? "trakt"
      : provider === "SIMKL"
        ? "simkl"
  ...
  const trackingPreferences: TrackingPreferences = {
    watchlistReadMode: normalizeTrackingReadMode(
      selection.watchlistReadMode,
      defaultMode,
    ),
    continueWatchingReadMode: normalizeTrackingReadMode(
      selection.continueWatchingReadMode,
      defaultMode,
    ),
    watchedReadMode: normalizeTrackingReadMode(
      selection.watchedReadMode,
      defaultMode,
    ),
    writeToTrakt:
      typeof selection.writeToTrakt === "boolean"
        ? selection.writeToTrakt
        : Boolean(traktToken),
    writeToSimkl:
      typeof selection.writeToSimkl === "boolean"
        ? selection.writeToSimkl
        : Boolean(simklToken),
  };
...
export async function saveCloudTrackingSelection(
  ...
  await mutateCloudPayload(auth, (root) => {
    ...
    const previousTraktAccessToken =
      previousTraktToken.accessToken ?? previousTraktToken.access_token;
    const writeTraktCredential =
      changedDomains.has("trakt") ||
      Boolean(
        incomingTraktAccessToken &&
        incomingTraktAccessToken !== previousTraktAccessToken,
      );
    ...
    const writeSimklCredential =
      changedDomains.has("simkl") ||
      Boolean(
        incomingSimklAccessToken &&
        incomingSimklAccessToken !== previousSimklAccessToken,
      );
    const defaultMode: TrackingReadMode =
      selection.provider === "TRAKT"
        ? "trakt"
    ...
    if (changedDomains.has("routing")) {
      Object.assign(nextSelection, {
        ...
        continueWatchingReadMode:
          preferences.continueWatchingReadMode.toUpperCase(),
...
export async function pullCloudWatchlist(
  auth: AuthClient,
  profileId?: string | null,
): Promise<MediaItem[]> {
  const root = await pullRawPayload(auth);
  ...
  const items = profileId
    ...
    : Object.values(byProfile).flatMap((value) =>
        arrayValue<AndroidWatchlistItem>(value),
      );
  return items
    .map((item): MediaItem | null => {
      ...
      if (!id) return null;
      const mediaType =
        String(item.mediaType ?? "movie").toLowerCase() === "tv"
          ? "tv"
          : "movie";
      return {
        id,
...
export async function saveCloudWatchlist(
  ...
  await mutateCloudPayload(auth, (root) => {
    const byProfile = objectRecord<unknown>(root.watchlistByProfile);
    byProfile[profileId] = items.map(
      (item, index): AndroidWatchlistItem => ({
        tmdbId: item.id,
...
function addonTypes(addon: InstalledAddon): string[] {
  return [
    ...
    ...addon.resources.flatMap((resource) =>
...
function isLiveOnlyAddon(addon: InstalledAddon): boolean {
  ...
  const hasStream = resourceNames.some(
    (name) => name === "stream" || name === "streams",
  );
  const hasCatalog =
  ...
  const text = [
    ...
    addon.name,
    addon.description ?? "",
    ...addon.catalogs.flatMap((catalog) => [
      catalog.type,
      catalog.id,
      catalog.name,
    ]),
    ...
    .toLowerCase();
  const hasSportsTerm =
    /(sport|football|soccer|basketball|tennis|racing|rugby|hockey|baseball|boxing|ufc|mma|cricket|golf)/.test(
      text,
    );
...
export function isLiveStreamOrSportsItem(
  item: {
  ...
  if (addonId) {
    const addon = addons.find(
      (candidate) => candidate.id.toLowerCase() === addonId,
    );
    if (addon) {
...
export async function removeContinueWatchingProgress(
  ...
  const matches = (candidate: AndroidContinueWatchingItem) => {
    ...
    if (item.seasonNumber != null && candidate.season !== item.seasonNumber)
      return false;
    if (item.episodeNumber != null && candidate.episode !== item.episodeNumber)
      return false;
    return true;
  };
...
export async function saveWatchedState(
  ...
  await mutateCloudPayload(auth, (root) => {
    if (item.mediaType === "movie") {
      ...
      const ids = new Set(
        arrayValue<number>(byProfile[targetProfileId])
          .map(Number)
          .filter((id) => id > 0),
      );
...
export async function markWatched(
  auth: AuthClient,
  entry: Omit<WatchHistoryEntry, "user_id" | "progress" | "position_seconds">,
  profileId?: string | null,
) {
