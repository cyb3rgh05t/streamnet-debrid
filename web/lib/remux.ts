...
export async function probeAndPrepareRemux(
  ...
  const send = (message: RemuxCommand) => {
  ...
  function event(
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(
        () => done(new Error(`Timed out waiting for ${name}`)),
        timeout,
      );
  ...
  // AC-3/E-AC-3 are deliberately excluded here even where MSE reports
  // isTypeSupported(...) === true for them: that flag only promises the
  // browser will accept the encoded bitstream into a buffer, which Chromium
  // uses for HDMI/AVR passthrough — on ordinary speakers the buffer is
  // accepted and playback "succeeds" with silence, no error anywhere. The
  // worker always registers a software AC-3/E-AC-3 decoder (mediabunny),
  // so leaving these codecs off this list forces a real transcode to AAC
  // instead of a passthrough that only works with bitstream-capable output.
  const audioCodecs = ["mp4a.40.2", "opus", "flac", "mp3"].filter((codec) =>
    Mse.isTypeSupported(`audio/mp4; codecs="${codec}"`),
  );
  ...
  try {
    probe = await new Promise<RemuxProbe>((resolve, reject) => {
      ...
      const cancel = () => finish();
      const timer = setTimeout(
        () => finish(undefined, "Source probe timed out"),
        15000,
      );
      cleanups.add(cancel);
      worker.onerror = () =>
        finish(undefined, "Browser conversion worker failed to load");
      ...
      send({
        ...
        audioCodecs,
        language: preferredAudioLang,
        expectDolbyVision: options.expectDolbyVision,
      });
    });
    probe.videoPlayable =
      probe.videoPlayable &&
      !!probe.videoCodec &&
      Mse.isTypeSupported(`video/mp4; codecs="${probe.videoCodec}"`);
    if (!probe.videoPlayable && !probe.videoReason)
      probe.videoReason = `This browser cannot decode the selected video track (${probe.videoCodec ?? "unknown codec"}).`;
  } catch (error) {
    destroy();
  ...
  const start = async (
    video: HTMLVideoElement,
    audioIndex = probe.chosenAudioIndex,
    startTime = 0,
  ) => {
    if (destroyed || element)
      throw new Error("Conversion is closed or already started");
    if (!probe.videoPlayable)
      throw new Error(
        probe.videoReason ?? "The browser cannot decode this video codec",
      );
    if (
      probe.audioTracks.length &&
      (audioIndex < 0 || !probe.audioTracks[audioIndex]?.browserPlayable)
    ) {
      throw new Error(
        "No compatible audio track; use server conversion or an external player",
      );
    }
    element = video;
    ...
    const codecs = [
      probe.videoCodec,
      audio ? (audio.passthrough ? audio.codec : "mp4a.40.2") : undefined,
    ].filter(Boolean);
    const mime = `video/mp4; codecs="${codecs.join(",")}"`;
    if (!Mse.isTypeSupported(mime))
      throw new Error("The browser cannot combine these video/audio codecs");
    source = new Mse();
    ...
    let target = Math.max(
      0,
      Math.min(startTime, Math.max(0, probe.duration - 0.1)),
    );
    ...
    const bufferedAt = (time: number) => {
      for (let i = 0; i < video.buffered.length; i++) {
        if (
          ...
          time + 0.25 < video.buffered.end(i)
        )
          return true;
      }
      return false;
    ...
    const begin = () =>
      send({ type: "start", generation, time: target, audioIndex });
    const onSeek = () => {
      if (
        destroyed ||
        (positionPending && Math.abs(video.currentTime - target) < 0.05) ||
        bufferedAt(video.currentTime)
      )
        return;
      ...
      pending = pending
        .then(async () => {
          ...
          if (buffer.buffered.length)
            await update(() => buffer!.remove(0, Infinity));
      ...
      pending = pending
        .then(async () => {
          ...
          if (
            ...
            buffer!.buffered.start(0) < before - 5
          ) {
    ...
    clockTimer = setInterval(
      () =>
        send({
          type: "clock",
          time: positionPending ? target : video.currentTime,
        }),
      250,
    );
...
export function remuxWorthTrying(url: string, text: string) {
  return (
    /^https?:\/\//i.test(url) &&
    /\.mkv(?:[?#/\s]|$)|\bmatroska\b|\bremux\b/i.test(`${url} ${text}`)
  );
}
