...
async function providerTextRequest(
  ...
  while (true) {
    ...
    if (
      ...
      budget.starts.length >= 30 ||
      now - queuedAt >= 5_000
    ) {
      throw new Error(
        "IPTV provider requests are temporarily paused. Please wait before refreshing.",
      );
    }
  ...
  budget.active++;
  try {
    return await textRequest(requestUrl, { cache: "no-store" });
  } catch (error) {
    const status = Number((error as { status?: number })?.status);
    const cooldown = [401, 403, 451].includes(status)
      ? 900_000
      : [429, 503, 513].includes(status)
        ? 300_000
...
export async function loadIptvSnapshot(
  ...
  const enabled = normalizedPlaylists.filter(
  ...
  const channelSets = await Promise.all(
    enabled.map(async (playlist) => {
      try {
        ...
        if (isXtream) {
          const xtreamChannels = await fetchXtreamChannels(
            playlist,
            options,
          ).catch(() => []);
        ...
        try {
          ...
          const xtreamChannels = isXtream
            ? []
            : await fetchXtreamChannels(playlist, options).catch(() => []);
          if (xtreamChannels.length) return xtreamChannels;
          throw m3uError;
        }
        ...
        playlistWarnings.push(
          `${playlist.name || playlist.m3uUrl}: ${error instanceof Error ? error.message : "Could not load playlist"}`,
        );
  ...
  const xtreamPlaylistIds = new Set(
    enabled
      .filter((playlist) => xtreamInfoFromPlaylist(playlist.m3uUrl))
      .map((playlist) => playlist.id),
  );
  const xmltvChannels = channels.filter(
    (channel) => !xtreamPlaylistIds.has(channel.id.split(":")[0] ?? ""),
  );
  ...
  const grouped = channels.reduce<Record<string, IptvChannel[]>>(
    (acc, channel) => {
  ...
  return {
    ...
    grouped: Object.fromEntries(
      orderedGroups.map((group) => [group, grouped[group]]),
    ),
    nowNext,
    favoriteChannels,
    favoriteGroups,
...
export async function loadIptvGuideForChannels(
  playlists: IptvPlaylistEntry[],
  ...
  const byPlaylist = new Map(
    enabled.map((playlist) => [playlist.id, playlist]),
  );
  const xtreamChannels: Array<{
    info: XtreamInfo;
    channel: IptvChannel;
    streamId: string;
  }> = [];
  ...
  const [xtreamGuide, xmltvGuide] = await Promise.all([
    ...
    xmltvChannels.length
      ? loadNowNext(enabled, xmltvChannels).catch(
          () => ({}) as Record<string, IptvNowNext>,
        )
      : Promise.resolve({} as Record<string, IptvNowNext>),
  ]);
...
function xtreamStreamIdFromUrl(url: string) {
  return (
    url.match(/\/(\d+)\.(?:m3u8|ts)(?:[?#]|$)/i)?.[1] ??
    url.match(/\/live\/[^/]+\/[^/]+\/(\d+)(?:[?#]|$)/i)?.[1] ??
    null
  );
}
...
async function loadXtreamGuide(
  entries: Array<{ info: XtreamInfo; channel: IptvChannel; streamId: string }>,
) {
  ...
  const results = await mapWithConcurrency(entries, 6, async (entry) => {
    const target = new URL(
      `${entry.info.baseUrl.replace(/\/+$/, "")}/player_api.php`,
    );
    ...
    const data = await fetchXtreamJson<{ epg_listings?: XtreamEpgListing[] }>(
      target.toString(),
  ...
  for (const result of results) {
    ...
    const sorted = result.programs.sort(
      (a, b) => a.startUtcMillis - b.startUtcMillis,
    );
    const live = sorted.find(
      (program) => now >= program.startUtcMillis && now < program.endUtcMillis,
    );
...
function xtreamInfoForChannel(
  playlists: IptvPlaylistEntry[],
  channel: IptvChannel,
) {
  const playlistId = channel.id.split(":")[0] ?? "";
  const playlist = normalizeIptvPlaylists(playlists).find(
...
export async function loadXtreamCatchup(
  playlists: IptvPlaylistEntry[],
  channel: IptvChannel,
  ...
  const data = await fetchXtreamJson<{
    epg_listings?: Array<XtreamEpgListing & { has_archive?: number | string }>;
  }>(target.toString(), playlistProxyHeaders()).catch(() => null);
  const now = Date.now();
...
export function buildXtreamCatchupUrl(
  ...
  const durationMinutes = Math.max(
    1,
    Math.round((program.endUtcMillis - program.startUtcMillis) / 60000),
  );
    ...
    async () => {
      ...
      const candidates: Array<{ label: string; load: () => Promise<string> }> =
        [
          ...
          {
            label: "proxy",
            load: () => providerTextRequest(url, playlistTextUrl(url)),
          },
          {
            label: "proxy with media headers",
            load: () => providerTextRequest(url, playlistTextUrl(url, headers)),
          },
        ];
      const errors: string[] = [];
      ...
      for (const candidate of candidates) {
        try {
          ...
          errors.push(
            `${candidate.label}: ${error instanceof Error ? error.message : "failed"}`,
          );
        }
      }
...
export function accessibleChannels(
  channels: IptvChannel[],
  hiddenGroups: string[],
) {
  const hidden = new Set(hiddenGroups);
  return channels.filter(
    (channel) =>
      !hidden.has(channel.group || "Uncategorized") &&
      !hidden.has(groupKey(channel)),
  );
}

export function normalizeIptvPlaylist(
  playlist: IptvPlaylistEntry,
  index = 0,
): IptvPlaylistEntry | null {
  const m3uUrl = normalizeIptvInput(playlist.m3uUrl);
  if (!m3uUrl) return null;
  const epgUrls = normalizeEpgInputs(
    [playlist.epgUrl, ...(playlist.epgUrls ?? [])].filter(Boolean).join("\n"),
  );
  return {
...
export function normalizeIptvInput(raw: string): string {
  ...
  if (creds && isXtreamUrl(parsed))
    return buildXtreamM3uUrl(
      xtreamBaseUrl(parsed),
      creds.username,
      creds.password,
    );
  return trimmed;
...
export function normalizeEpgInputs(raw: string): string[] {
  ...
  if (triplet)
    return [
      buildXtreamEpgUrl(triplet.baseUrl, triplet.username, triplet.password),
    ];
  const urls = trimmed.match(/https?:\/\/[^\s,;|]+/gi);
  if (urls && urls.length > 1)
    return [...new Set(urls.flatMap(normalizeEpgInputs))];
  const parts = trimmed
    .split(/[\n\r,;|]+/)
    .map((part) => part.trim())
    .filter(Boolean);
  ...
  if (creds && isXtreamUrl(parsed))
    return [
      buildXtreamEpgUrl(xtreamBaseUrl(parsed), creds.username, creds.password),
    ];
  return [trimmed];
}
...
function decodeLegacyBase64Url(value: string): string | null {
  if (value.length < 12 || value.includes("://") || /\s/.test(value))
    return null;
  try {
...
async function fetchXtreamChannels(
  ...
  const headers = playlistProxyHeaders(options.userAgent);
  const streamsUrl = buildXtreamPlayerApiUrl(
    info.baseUrl,
    info.username,
    info.password,
    "get_live_streams",
  );
  const categoriesUrl = buildXtreamPlayerApiUrl(
    info.baseUrl,
    info.username,
    info.password,
    "get_live_categories",
  ...
  const [streams, categories] = await Promise.all([
    ...
    fetchXtreamJson<XtreamCategory[]>(categoriesUrl, headers).catch(
      () => [] as XtreamCategory[],
    ),
  ]);
  ...
  return streams.flatMap((stream) => {
    ...
    seen.add(id);
    const group =
      groupById.get(String(stream.category_id ?? "")) || "Uncategorized";
    ...
    const archiveDays = Number(stream.tv_archive)
      ? Number(stream.tv_archive_duration) || 1
      : 0;
    return [
      {
        id,
  ...
  const task = (async () => {
    const load = async () => {
      try {
        ...
        const text = await providerTextRequest(
          url,
          playlistTextUrl(url, headers),
        );
        jsonFromText(text);
        return text;
      }
    ...
    const guide =
      action === "get_short_epg" || action === "get_simple_data_table";
    const text = guide
      ? await load()
      : await cachedRemoteText(`api:${key}`, PLAYLIST_TTL_MS, load);
    const data = jsonFromText<T>(text);
    if (guide) {
      const empty = !(data as { epg_listings?: unknown[] })?.epg_listings
        ?.length;
      guideApiCache.set(key, {
        data,
        expires: Date.now() + (empty ? 600_000 : 120_000),
      });
      while (guideApiCache.size > 128)
        guideApiCache.delete(guideApiCache.keys().next().value!);
    }
    return data;
  })();
...
function jsonFromText<T>(text: string) {
  if (isHtmlText(text))
    throw new Error(
      "The remote service returned an HTML error page instead of API data.",
    );
...
function assertPlaylistText(text: string) {
  ...
  throw new Error(
    "The provider returned data, but it was not an M3U playlist.",
  );
}
...
function orderGroups(
  ...
  const firstChannelByGroup = channels.reduce<Record<string, IptvChannel>>(
    (acc, channel) => {
      const group = channel.group || "Uncategorized";
  ...
  for (const rawLine of lines) {
    ...
    if (/^#EXTINF:/i.test(line)) {
      ...
      pending = {
        ...
        catchupDays:
          attr(metadata, "catchup-days") || attr(metadata, "timeshift"),
        catchupType: attr(metadata, "catchup"),
        catchupSource: attr(metadata, "catchup-source"),
        language: firstAttr(metadata, [
          "tvg-language",
          "tvg-lang",
          "language",
          "lang",
        ...
        qualityLabel: firstAttr(metadata, [
          "quality",
          "tvg-quality",
          "resolution",
        ]),
      ...
      channels.push({
        ...
        qualityLabel:
          pending.qualityLabel ||
          inferQualityLabel(pending.name, pending.group),
        streamUrl,
...
export function migrateXtreamFavoriteIds(
  favorites: string[],
  channels: IptvChannel[],
): string[] {
  const legacyPrefixes = new Set(
    favorites
      .filter((id) => id.includes(":m3u:"))
  ...
  for (const channel of channels) {
    ...
    for (const url of [
      channel.streamUrl,
      channel.streamUrl.replace(/\.m3u8(?=[?#]|$)/i, ".ts"),
    ]) {
...
export function isDividerChannelName(name?: string) {
  ...
  return (
    /^[#=\-*•_~|<>♦►]{3,}/.test(trimmed) && /[#=\-*•_~|<>♦►]{3,}$/.test(trimmed)
  );
}
...
async function loadNowNext(
  ...
  for (const playlist of playlists) {
    ...
    const urls = [
      ...new Set(
        [playlist.epgUrl, ...(playlist.epgUrls ?? [])].filter(
  ...
  return Object.fromEntries(
    Object.entries(programsById).map(([channelId, programs]) => {
      const sorted = [
        ...new Map(programs.map((p) => [p.startUtcMillis, p])).values(),
      ].sort((a, b) => a.startUtcMillis - b.startUtcMillis);
      const live = sorted.find(
        (program) =>
      ...
      const recent = sorted.filter(
        (program) =>
          program.endUtcMillis <= now &&
          program.endUtcMillis >= now - 48 * 3600_000,
      );
      return [
        channelId,
        {
          ...
          next: future[0],
          later: future[1],
          upcoming: future.filter(
            (program) => program.startUtcMillis < now + 48 * 3600_000,
          ),
          recent,
...
const parsedGuides = new Map<
  string,
  { at: number; promise: Promise<ReturnType<typeof parseXmltv>> }
>();
function cachedXmltvPrograms(url: string) {
  ...
  const promise = fetchEpgText(url)
    .then(parseXmltv)
    .catch((error) => {
      parsedGuides.delete(url);
      throw error;
    });
  if (parsedGuides.size >= 4)
    parsedGuides.delete(parsedGuides.keys().next().value!);
  ...
  while ((match = programRe.exec(xml))) {
    ...
    results.push({
      ...
      program: {
        ...
        artworkUrl: safeGuideImage(
          decodeXml(
            attr(body.match(/<icon\b([^>]*)\/?\s*>/i)?.[1] ?? "", "src"),
          ),
        ),
        category:
          decodeXml(textTag(body, "category") || "").slice(0, 200) || undefined,
...
const identityLoads = new Map<
  string,
  { expires: number; task: Promise<IptvChannel[]> }
>();

/** A single cached metadata request, never a stream probe or a request per channel. */
export async function loadIptvChannelIdentities(
  playlists: IptvPlaylistEntry[],
  channels: IptvChannel[],
  options: IptvLoadOptions = {},
) {
  const replacements = new Map<string, IptvChannel>();
  for (const playlist of normalizeIptvPlaylists(playlists).filter(
    (p) => p.enabled,
  )) {
    const scoped = channels.filter((c) =>
      c.id.startsWith(`${playlist.id}:xtream:`),
    );
    ...
    if (!cached || cached.expires < Date.now()) {
      const task = fetchPlaylistText(url.toString(), options).then((text) =>
        attachM3uChannelIdentities(scoped, text, playlist.id),
      );
      ...
      void task.catch(() => {
        if (identityLoads.get(key) === cached)
          cached!.expires = Date.now() + 300_000;
      });
      if (identityLoads.size > 12)
        identityLoads.delete(identityLoads.keys().next().value!);
    }
    try {
      for (const channel of await cached.task)
        replacements.set(channel.id, channel);
    } catch {
      /* Keep API channels available when the optional identity bridge fails. */
    }
  }
  return channels.map((channel) => {
    const identity = replacements.get(channel.id);
    return identity?.cloudId
      ? {
          ...channel,
          cloudId: identity.cloudId,
          syncAliases: identity.syncAliases,
        }
      : channel;
  });
}

export async function attachM3uChannelIdentities(
  channels: IptvChannel[],
  text: string,
  playlistId: string,
) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (channel) {
      ...
      replacements.set(id, {
        ...channel,
        cloudId: previous?.cloudId ?? cloudId,
        syncAliases: [
          ...new Set([...(previous?.syncAliases ?? []), cloudId, hlsAlias]),
        ],
      });
    }
    epgId = undefined;
...
async function cachedRemoteText(
  key: string,
  ttlMs: number,
  ...
  if (cached && cachedIsValid && Date.now() - cached.cachedAt < ttlMs)
    return cached.text;
  try {
    const fresh = await loader();
    if (validator && !validator(fresh))
      throw new Error("The remote response was not valid data.");
...
function inferQualityLabel(name?: string, group?: string) {
  ...
  if (text.includes("UHD") || text.includes("4K") || text.includes("2160"))
    return "4K";
...
export function iptvPlaylistSignature(playlists: IptvPlaylistEntry[]): string {
  return playlists
    .map(
      (playlist) =>
        `${playlist.id}:${playlist.enabled}:${playlist.m3uUrl}:${playlist.epgUrl ?? ""}:${playlist.epgUrls?.join("|") ?? ""}`,
    )
    .join("||");
}
