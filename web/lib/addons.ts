import { HttpError, jsonRequest, proxiedUrl } from "./http";
...
function addonBaseUrl(manifestUrl: string) {
  try {
    ...
    return {
      base: path.replace(/\/manifest\.json$/, "").replace(/\/+$/, ""),
      query,
    };
  }
}
...
function manifestUrlFor(addon: InstalledAddon) {
  const compatible = addon as InstalledAddon & {
    url?: string;
    transportUrl?: string;
  };
  const raw =
    compatible.manifestUrl || compatible.url || compatible.transportUrl || "";
...
function normalizeCatalogs(catalogs: unknown) {
  return Array.isArray(catalogs)
    ? catalogs.filter((catalog): catalog is AddonCatalog =>
        Boolean(catalog && typeof catalog === "object"),
      )
    : [];
}
...
export function normalizeAddon(addon: unknown): InstalledAddon | null {
  ...
  return {
    ...
    version:
      typeof raw.version === "string" && raw.version ? raw.version : "1.0.0",
    manifestUrl,
...
export async function installAddon(rawUrl: string) {
  ...
  const resourceNames = (manifest.resources ?? [])
    .map((resource) =>
      typeof resource === "string"
        ? resource
        : ((resource as { name?: string } | null)?.name ?? ""),
    )
    .filter(Boolean);
  const subtitleOnly =
    resourceNames.includes("subtitles") && !resourceNames.includes("stream");
        ...
        .map((resource) =>
          typeof resource === "string"
            ...
            : {
                ...
                types: (resource as { types?: string[] })?.types ?? [],
                idPrefixes:
                  (resource as { idPrefixes?: string[] })?.idPrefixes ?? null,
              },
        )
        .filter((resource) => resource.name),
      types: manifest.types ?? [],
      idPrefixes: manifest.idPrefixes ?? [],
      catalogs: (Array.isArray(manifest.catalogs) ? manifest.catalogs : []).map(
        (catalog) => ({
          type: String((catalog as { type?: string })?.type ?? "movie"),
          id: String((catalog as { id?: string })?.id ?? ""),
          name: String(
            (catalog as { name?: string })?.name ??
              (catalog as { id?: string })?.id ??
              "",
          ),
        }),
      ),
...
export async function getStreamsProgressive(
  ...
  if (hasResolverConfig()) {
    const browserPromise = getBrowserStreamsProgressive(
      addons,
      item,
      season,
      episode,
    ...
    const resolverPromise = getResolverStreamsProgressive(
      addons,
      item,
      season,
      episode,
      ...
      console.warn(
        "ARVIO resolver unavailable, using browser addon lookup.",
        error,
      );
      return [] as StreamSource[];
    });
    const [browserResolved, resolved] = await Promise.all([
      browserPromise,
      resolverPromise,
    ]);
    const merged = sortStreams(
      dedupeStreams([...browserResolved, ...resolved]).filter(
        (stream) =>
          stream.url || stream.infoHash || stream.description || stream.source,
      ),
    );
...
async function getBrowserStreamsProgressive(
  ...
  const subtitleAddons = addons.filter(
    (addon) =>
      addon.enabled !== false &&
      manifestUrlFor(addon) &&
      supportsResource(addon, "subtitles"),
  );
  ...
  if (
    !subtitleAddons.some(
      (addon) => manifestUrlFor(addon) === BUILTIN_OPENSUBTITLES.manifestUrl,
    )
  ) {
  ...
  const subtitlePromise = queryAllAddonSubtitles(subtitleAddons, type, ids)
    .then((subtitles) => {
      ...
      if (subtitles.length && aggregate.length) {
        aggregate = aggregate.map((stream) =>
          attachSubtitles(stream, subtitles),
        );
        onUpdate?.(aggregate, []);
      }
      return subtitles;
  ...
  await Promise.all(
    enabled.map(async (addon) => {
      ...
      const withSubtitles = batch.map((stream) =>
        attachSubtitles(stream, sharedSubtitles),
      );
      aggregate = sortStreams(
        dedupeStreams([...aggregate, ...withSubtitles]).filter(
          (stream) =>
            stream.url ||
            stream.infoHash ||
            stream.description ||
  ...
  if (finalSubtitles.length)
    aggregate = aggregate.map((stream) =>
      attachSubtitles(stream, finalSubtitles),
    );
  return aggregate;
}

async function queryAddonStreams(
...
async function queryAllAddonSubtitles(
  addons: InstalledAddon[],
  type: "movie" | "series",
  ids: string[],
) {
  const batches = await Promise.all(
    addons.map((addon) => queryAddonSubtitles(addon, type, ids)),
  );
  const seen = new Set<string>();
  return batches
    .flat()
...
async function queryAddonSubtitles(
  addon: InstalledAddon,
  type: "movie" | "series",
  ids: string[],
) {
  ...
  for (const id of ids) {
    ...
    try {
      const payload = await addonJsonRequest<{ subtitles?: RawSubtitle[] }>(
        url,
      );
...
function isAddonAuthorizationError(error: unknown) {
...
function streamIds(item: MediaItem, season?: number, episode?: number) {
  ...
  ids.push(
    hasEpisode ? `tmdb:${item.id}:${season}:${episode}` : `tmdb:${item.id}`,
  );
...
function streamRequestTypes(
  ...
  const aliases =
    requestedType === "series" ? ["series", "tv", "show"] : ["movie", "film"];
...
function addonSupportsType(addon: InstalledAddon, requestedType: string) {
  ...
  const add = (value: unknown) => {
    if (typeof value === "string" && value.trim())
      declared.add(value.trim().toLowerCase());
  };
...
function addonSupportsId(addon: InstalledAddon, id: string) {
  const prefixes = [
    ...
    ...(addon.resources ?? []).flatMap((resource) =>
      typeof resource === "string" ? [] : (resource.idPrefixes ?? []),
    ),
  ]
...
function normalizeStream(
  ...
  const text = [stream.title, stream.name, stream.description, stream.size]
    .filter(Boolean)
    .join(" ");
  ...
  const normalized: StreamSource = {
    ...
    size:
      stream.size ??
      sizeLabel(stream.sizeBytes ?? stream.behaviorHints?.videoSize ?? null),
    ...
    behaviorHints: {
      ...
      notWebReady:
        !url &&
        Boolean(
          stream.infoHash || stream.ytId || stream.behaviorHints?.videoHash,
        ),
    },
...
function attachSubtitles(
  ...
  const merged = [
    ...existing,
    ...sharedSubtitles.filter((subtitle) => !seen.has(subtitle.url)),
  ];
  return merged.length === existing.length
    ? stream
    : { ...stream, subtitles: merged };
}
