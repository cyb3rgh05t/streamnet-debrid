...
let activeTmdbLanguage = "de-DE";

export function setTmdbLanguage(language: "de-DE" | "en-US"): void {
  activeTmdbLanguage = language;
}
...
type TmdbPersonResponse = {
  ...
  combined_credits?: {
    ...
    >;
    crew?: Array<
      TmdbItem & { job?: string; vote_count?: number; popularity?: number }
    >;
  };
};
...
export function isAnime(item: TmdbItem | Partial<MediaItem>): boolean {
  if ("isAnime" in item && typeof item.isAnime === "boolean")
    return item.isAnime;
  const genreIds =
    ("genre_ids" in item
      ? item.genre_ids
  ...
  const hasAnimation =
    ...
    (genres as any[]).some((g: any) =>
      typeof g === "string"
        ? g.toLowerCase() === "animation"
        : g.id === 16 || g.name === "Animation",
    );
  const origLang =
    ("original_language" in item
      ? item.original_language
      : "originalLanguage" in item
...
export function mapTmdbItem(
  ...
  const mediaType: MediaType =
    item.media_type === "tv" || fallbackType === "tv" ? "tv" : "movie";
...
export async function tmdb<T>(
  ...
  const releaseSlot = await acquireTmdbSlot();
...
export async function loadHomeCategories(
  ...
  if (catalogs?.length) {
    ...
    const rows = await Promise.all(
      enabled.map((catalog) =>
        loadCatalog(catalog, language).catch(() => null),
      ),
    );
    const hydrated = rows.filter((row): row is Category =>
      Boolean(row?.items.length),
    );
    ...
    return [
      {
        ...
        items: movies.results.map((x) => mapTmdbItem(x, "movie")),
      },
      {
        id: "trending_tv",
        title: "Trending in Shows",
        items: series.results.map((x) => mapTmdbItem(x, "tv")),
      },
      {
        id: "trending_anime",
        title: "Trending in Anime",
        items: anime.results.map((x) => mapTmdbItem(x, "tv")),
      },
      {
        id: "popular_movies",
        title: "Popular Movies",
        items: popularMovies.results.map((x) => mapTmdbItem(x, "movie")),
      },
      {
        id: "popular_tv",
        title: "Popular Series",
        items: popularTv.results.map((x) => mapTmdbItem(x, "tv")),
      },
    ];
  } catch {
...
export async function loadCatalog(
  ...
  if (isCollectionCatalog(catalog)) {
    const items = await loadCollectionCatalog(
      catalog,
      language,
      addons,
      homeServers,
  ...
  if (catalog.sourceType === "tmdb" && catalog.endpoint) {
    ...
    return {
      ...
      title: catalog.name,
      items: results.map((x) =>
        mapTmdbItem(x, catalog.mediaType === "tv" ? "tv" : "movie"),
      ),
      sourceLabel: "TMDB",
      layout: catalog.layout ?? "landscape",
    };
  ...
  if (catalog.sourceType === "trakt" && catalog.sourceUrl) {
    const refs = (await loadTraktPublicList(catalog.sourceUrl)) as Array<{
      type: MediaType;
      id: number;
    }>;
  ...
  if (catalog.sourceType === "home-server") {
    const { loadHomeServerLibraryItems, loadHomeServerRows } =
      await import("./homeserver");
    const servers = homeServers ?? [];
    if (catalog.sourceUrl?.startsWith("hslib:")) {
      const items = await loadHomeServerLibraryItems(
        servers,
        catalog.sourceUrl,
      );
      return {
    ...
    const row = rows.find(
      (r) =>
        r.id === catalog.id ||
        r.title.toLowerCase().includes(catalog.name.toLowerCase()),
    );
    return {
      id: catalog.id,
      title: catalog.name,
...
function isCollectionCatalog(catalog: CatalogConfig) {
  ...
  return (
    ...
    Boolean(catalog.collectionSources?.length)
  );
}
...
async function loadCollectionCatalog(
  ...
  if (!sources.length) {
    if (catalog.sourceType === "mdblist" && catalog.sourceUrl)
      return loadMdblist(catalog, language);
    return [];
  }
  const batches = await Promise.all(
    sources.map((source) =>
      loadCollectionSource(source, language, addons, homeServers).catch(
        () => [],
      ),
    ),
  );
...
async function loadCollectionSource(
  ...
  if (kind === "CURATED_IDS") {
    ...
    const items = await Promise.all(
      refs.map((ref) =>
        getBasicItem(ref.type, ref.id, language).catch(() => null),
      ),
    );
  ...
  if (kind === "TMDB_COLLECTION" && source.tmdbCollectionId) {
    const response = await tmdb<{ parts?: TmdbItem[] }>(
  ...
  if (kind === "TMDB_PERSON" && source.tmdbPersonId) {
    const response = await tmdb<TmdbCombinedCredits>(
      `person/${source.tmdbPersonId}/combined_credits`,
      { language },
    );
    return dedupeItems(
  ...
  if (kind === "TMDB_GENRE" && source.tmdbGenreId) {
    return loadTmdbCatalogPages(
      ...
      items.map((item) =>
        mapTmdbItem(item, mediaType === "tv" ? "tv" : "movie"),
      ),
    );
  }
    ...
    return loadAddonCatalog(
      {
        ...
        sourceUrl: source.addonManifestUrl ?? undefined,
  ...
  if (kind === "HOME_SERVER" || kind === "HOMESERVER") {
    ...
    if (source.homeServerLibraryKey) {
      return loadHomeServerLibraryItems(
        servers,
        `hslib:${source.homeServerId ?? ""}:${source.homeServerLibraryKey}`,
      );
    }
    const rows = await loadHomeServerRows(servers);
...
function sourceMediaType(source: CollectionSourceConfig): MediaType | "all" {
  const raw = String(source.mediaType ?? "")
    .trim()
    .toLowerCase();
...
function catalogPageLimit(catalog: CatalogConfig) {
  const raw = Number(
    catalog.params?.pageLimit ??
      catalog.params?.pages ??
      TMDB_CATALOG_PAGE_LIMIT,
  );
...
async function loadTmdbCatalogPages(catalog: CatalogConfig, language: string) {
  ...
  const first = await tmdb<TmdbList>(catalog.endpoint, {
    ...params,
    language,
    page: startPage,
  });
  const totalPages = Math.max(
    startPage,
    Number(first.total_pages ?? startPage) || startPage,
  );
  const lastPage = Math.min(
    totalPages,
    startPage + catalogPageLimit(catalog) - 1,
  );
  const pageRequests: Array<Promise<TmdbList>> = [];
  for (let page = startPage + 1; page <= lastPage; page += 1) {
    pageRequests.push(
      tmdb<TmdbList>(catalog.endpoint, { ...params, language, page }).catch(
        () => ({ results: [] }),
      ),
    );
  }
  const rest = await Promise.all(pageRequests);
  return dedupeTmdbItems([
    ...(first.results ?? []),
...
async function hydrateRefs(
  refs: Array<{ type: MediaType; id: number }>,
  language: string,
) {
  ...
  for (
    let index = 0;
    index < firstPaintRefs.length;
    index += HYDRATE_BATCH_SIZE
  ) {
    const chunk = firstPaintRefs.slice(index, index + HYDRATE_BATCH_SIZE);
    const details = await Promise.all(
      chunk.map((item) =>
        getBasicItem(item.type, item.id, language).catch(() => null),
      ),
    );
    output.push(
      ...details.filter((item): item is MediaItem => Boolean(item?.title)),
    );
  }
...
async function loadMdblist(catalog: CatalogConfig, language: string) {
  ...
  const ids = rawItems
    .map((item) =>
      extractMdblistIdentity(item as MdblistItem, catalog.mediaType),
    )
    .filter(<T>(item: T | null | undefined): item is NonNullable<T> =>
      Boolean((item as any)?.id),
    );
...
function parseTraktUrl(
  ...
  | { type: "user"; user: string; slug: string }
  ...
  try {
    ...
    if (usersIndex >= 0 && parts[usersIndex + 1] && parts[usersIndex + 3]) {
      return {
        type: "user",
        user: parts[usersIndex + 1],
        slug: parts[usersIndex + 3],
      };
    }
...
function extractMdblistIdentity(
  item: MdblistItem,
  preferred?: CatalogConfig["mediaType"],
) {
  const ids =
    objectValue(item, "ids") ??
    objectValue(objectValue(item, "movie"), "ids") ??
    objectValue(objectValue(item, "show"), "ids");
  ...
  const type: MediaType =
    rawType.includes("show") ||
    rawType.includes("series") ||
    rawType === "tv" ||
    (!rawType && preferred === "tv")
      ? "tv"
      : "movie";
  return { id: tmdb, type };
}
...
export function resolveTmdbId(item: {
  ...
  if ((!imdbId || !/^tt\d+$/.test(imdbId)) && !normalizedTitle)
  ...
  const request = (async () => {
    if (imdbId && /^tt\d+$/.test(imdbId)) {
      const payload = await tmdb<TmdbExternalFind>(`find/${imdbId}`, {
        external_source: "imdb_id",
      }).catch(() => null);
      const externalResults =
        item.mediaType === "tv" ? payload?.tv_results : payload?.movie_results;
    ...
    const best = search?.results
      .map((candidate) => {
        ...
        const candidateYear =
          Number(
            yearFrom(candidate.release_date ?? candidate.first_air_date),
          ) || null;
        const yearScore =
          !year || !candidateYear
            ? 0
...
function restoreLogoCache() {
  ...
  try {
    ...
    if (!raw) return;
    Object.entries(JSON.parse(raw) as Record<string, string | null>).forEach(
      ([k, v]) => logoCache.set(k, v),
    );
  } catch {
    /* ignore */
  }
...
export async function getLogoUrl(item: {
  mediaType: MediaType;
  id: number;
}): Promise<string | null> {
...
function restoreProviderCache() {
  ...
  try {
    ...
    if (!raw) return;
    Object.entries(JSON.parse(raw) as Record<string, string[]>).forEach(
      ([k, v]) => providerCache.set(k, v),
    );
  } catch {
    /* ignore */
  }
...
function restoreCardMetaCache() {
  ...
  try {
    ...
    if (raw)
      Object.entries(JSON.parse(raw) as Record<string, CardMeta>).forEach(
        ([k, v]) => cardMetaCache.set(k, v),
      );
  } catch {
    /* ignore */
  }
...
export async function getCardMeta(item: {
  mediaType: MediaType;
  id: number;
}): Promise<{
  runtime: number;
...
export async function getCardProviders(item: {
  mediaType: MediaType;
  id: number;
}): Promise<string[]> {
  const key = `${item.mediaType}:${item.id}`;
...
function providersBeforeTmdb(
  type: MetadataMediaType,
  configOverride?: ProviderPriorityConfig,
): ProviderPriorityConfig | null {
  const priority = MetadataDispatcher.getPriorityList(type, configOverride);
  const tmdbIndex = priority.indexOf("tmdb");
  const providers = priority.slice(
    0,
    tmdbIndex >= 0 ? tmdbIndex : priority.length,
  );
  if (providers.length === 0) return null;

  const movieProviders: MetadataProviderId[] =
    configOverride?.movieProviders ?? ["tmdb"];
  const tvProviders: MetadataProviderId[] = configOverride?.tvProviders ?? [
    "tvdb",
    "tmdb",
  ];
  const animeProviders: MetadataProviderId[] =
    configOverride?.animeProviders ?? ["anilist", "tvdb", "tmdb"];
  return {
    ...configOverride,
...
export async function getSeasonEpisodes(
  ...
  const metadataType: MetadataMediaType = metadataContext?.isAnime
    ? "anime"
    : "tv";
  ...
  const externalEpisodesPromise =
    externalConfig && metadataType === "tv"
      ? MetadataDispatcher.getEpisodes(
          lookup,
          metadataType,
          seasonNumber,
          externalConfig,
  ...
  const enrichEpisodes = async (episodes: EpisodeInfo[]) => {
    ...
    return episodes.map((episode) => {
      const external = externalByNumber.get(
        `${episode.seasonNumber}:${episode.episodeNumber}`,
      );
  ...
  try {
    ...
    registerSeasonEpisodeNumbers(
      tvId,
      seasonNumber,
      (season.episodes ?? []).map((e) => e.episode_number).filter(Boolean),
    );
    const ratingsPromise = getSeasonEpisodeRatings(tvId, seasonNumber).catch(
    ...
    const episodes = (season.episodes ?? []).map((episode) => ({
      ...
      still: episode.still_path
        ? `${config.imageBase}${episode.still_path}`
        : undefined,
      voteAverage: episode.vote_average ?? 0,
      imdbRating:
        imdbRatings.get(`${seasonNumber}:${episode.episode_number}`) ?? "",
...
function readSeasonEpisodesCache(key: string) {
  const cache = loadStored<
    Record<string, { at: number; episodes: EpisodeInfo[] }>
  >(SEASON_EPISODE_CACHE_KEY, {});
  ...
  return Array.isArray(entry.episodes) && entry.episodes.length
    ? entry.episodes
    : null;
}
...
export async function getReviews(item: {
  mediaType: MediaType;
  id: number;
}): Promise<ReviewInfo[]> {
  try {
...
async function getSeasonEpisodeRatings(tvId: number, seasonNumber: number) {
  ...
  if (seriesEpisodeRatingsCache.has(cacheKey))
    return seriesEpisodeRatingsCache.get(cacheKey)!;
  ...
  try {
    ...
    const workers = Array.from(
      { length: Math.min(8, episodeNumbers.length) },
      async () => {
        while (cursor < episodeNumbers.length) {
          ...
          if (/^tt\d+$/.test(raw))
            slots.push({ key: `${seasonNumber}:${number}`, imdbId: raw });
        }
      },
    );
...
export function registerSeasonEpisodeNumbers(
  tvId: number,
  seasonNumber: number,
  numbers: number[],
) {
...
export async function getBasicItem(
  mediaType: MediaType,
  id: number,
  language = "en-US",
): Promise<MediaItem | null> {
...
const detailsPayloadCache = new Map<
  string,
  { at: number; payload: TmdbItem }
>();
const DETAILS_CACHE_TTL_MS = 10 * 60 * 1000;

async function fetchDetailsPayload(item: MediaItem, customApiKey?: string) {
  const key = `${activeTmdbLanguage}-${item.mediaType}-${item.id}`;
  const cached = detailsPayloadCache.get(key);
  if (cached && Date.now() - cached.at < DETAILS_CACHE_TTL_MS)
    return cached.payload;
  const payload = await tmdb<TmdbItem>(
    `${item.mediaType}/${item.id}`,
    {
      language: activeTmdbLanguage,
      append_to_response:
        "credits,videos,similar,recommendations,external_ids,watch/providers",
    },
    customApiKey,
  );
  detailsPayloadCache.set(key, { at: Date.now(), payload });
  if (detailsPayloadCache.size > 60) {
    const oldest = [...detailsPayloadCache.entries()].sort(
      (a, b) => a[1].at - b[1].at,
    )[0];
...
export async function getDetails(
  ...
  try {
    const details = await fetchDetailsPayload(
      item,
      priorityConfig?.customTmdbApiKey,
    );
    const mapped = mapTmdbItem(
      { ...details, media_type: item.mediaType },
      item.mediaType,
    );
    ...
    if (externalConfig) {
      ...
      resolvedMeta = await MetadataDispatcher.getDetails(
        lookup,
        metadataType,
        externalConfig,
      ).catch(() => null);
    ...
    const related = [
      ...
      .filter(
        (candidate, index, arr) =>
          arr.findIndex((x) => x.id === candidate.id) === index,
      )
      .slice(0, 18)
      .map((candidate) =>
        mapTmdbItem(
          { ...candidate, media_type: item.mediaType },
          item.mediaType,
        ),
      );
    return {
          ...
          ? (details.seasons ?? [])
              ...
              .map((season) => ({
                ...
                poster: season.poster_path
                  ? `${config.imageBase}${season.poster_path}`
                  : "",
              }))
          : [],
      related,
...
const fallbackItems: MediaItem[] = [
  {
    ...
    subtitle: "S1 E6",
    overview:
      "Long-buried secrets surface as a small crew follows a signal across a collapsing horizon.",
    year: "2026",
    rating: "8.7",
    duration: "45m",
    mediaType: "tv",
  ...
  },
  {
    ...
    overview:
      "A detective chases a synthetic witness through a city where memory is traded like currency.",
    year: "2025",
    rating: "8.2",
    duration: "2h 10m",
    mediaType: "movie",
  ...
  },
  {
    ...
    subtitle: "S2 E3",
    overview:
      "A survival drama moves into stranger terrain when the map stops matching the world.",
    year: "2026",
    rating: "8.0",
    duration: "50m",
    mediaType: "tv",
  ...
  },
  {
    ...
    overview:
      "A retired planner is pulled back into the city he designed to stop a hostile takeover.",
    year: "2025",
    rating: "7.9",
    duration: "1h 58m",
    mediaType: "movie",
...
const fallbackCategories: Category[] = [
  {
    id: "continue_watching_demo",
    title: "Continue Watching",
    items: fallbackItems.filter((item) => item.progress),
  },
  {
    id: "trending_movies_demo",
    title: "Trending in Movies",
    items: fallbackItems.filter((item) => item.mediaType === "movie"),
  },
  {
    id: "trending_tv_demo",
    title: "Trending in Shows",
    items: fallbackItems.filter((item) => item.mediaType === "tv"),
  },
];
