...
title StreamNet - Enable "Open in VLC"
echo.
echo   ================================================
echo    StreamNet  -  Enable "Open in VLC" for your browser
echo   ================================================
echo.
echo   This registers the vlc:// link handler so the "Open in VLC"
echo   button on web.streamnet.live opens streams directly in VLC, with no
...
set "HANDLERDIR=%LOCALAPPDATA%\StreamNet"
if not exist "%HANDLERDIR%" mkdir "%HANDLERDIR%" >nul 2>&1
if not exist "%HANDLERDIR%" goto :handler_failed

set "HANDLER=%HANDLERDIR%\streamnet-vlc.ps1"
set "STREAMNET_INSTALLER_PATH=%~f0"
set "STREAMNET_HANDLER_PATH=%HANDLER%"
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$lines = Get-Content -LiteralPath $env:STREAMNET_INSTALLER_PATH; $marker = [Array]::IndexOf($lines, ':STREAMNET_POWERSHELL_HANDLER'); if ($marker -lt 0 -or $marker -ge ($lines.Count - 1)) { exit 1 }; $utf8 = New-Object Text.UTF8Encoding($false); [IO.File]::WriteAllLines($env:STREAMNET_HANDLER_PATH, $lines[($marker + 1)..($lines.Count - 1)], $utf8)"
...
> "%HANDLERDIR%\streamnet-vlc.bat" echo @echo off
if errorlevel 1 goto :handler_failed
>>"%HANDLERDIR%\streamnet-vlc.bat" echo powershell.exe -NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File "%%~dp0streamnet-vlc.ps1" "%%~1"
if errorlevel 1 goto :handler_failed
...
echo   ================================================
echo    Done!  "Open in VLC" now works on web.streamnet.live.
echo   ================================================
echo.
echo   Go back to StreamNet and click "Open in VLC" on any source.
