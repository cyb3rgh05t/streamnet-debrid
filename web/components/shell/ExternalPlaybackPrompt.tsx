...
function labelFor(pending: PendingExternalPlayback, language: UiLanguage) {
  const episode =
    pending.mediaType === "tv" && pending.season && pending.episode
      ? `S${pending.season} E${pending.episode}${pending.episodeTitle ? ` - ${pending.episodeTitle}` : ""}`
      : localize(language, "Film", "Movie");
  return `${pending.title} - ${episode}`;
}

export function ExternalPlaybackPrompt() {
  const { settings, addons, refreshData, setToast, markWatchedLocally } =
    useApp();
    ...
    try {
      ...
      setToast(
        finished
          ? localize(
              settings.uiLanguage,
              "Als gesehen markiert und synchronisiert.",
              "Marked watched and synced.",
            )
          : localize(
              settings.uiLanguage,
  ...
  return (
    <section
      className="external-playback-modal"
      role="dialog"
      aria-modal="true"
      aria-label={localize(
        settings.uiLanguage,
        "Fortschritt externer Wiedergabe",
        "External playback tracking",
      )}
    >
      ...
      <article className="external-playback-card">
        <button
          type="button"
          className="external-playback-close"
          onClick={() => dismiss()}
          aria-label={localize(
            settings.uiLanguage,
            "Abfrage zur externen Wiedergabe schließen",
            "Close external playback prompt",
          )}
        >
          <X size={18} />
        </button>
        <p className="eyebrow">
          {pending.player === "vlc" ? "VLC" : "Infuse"}{" "}
          {localize(settings.uiLanguage, "Wiedergabe", "Playback")}
        </p>
        <h3>
          {localize(
            settings.uiLanguage,
            "Wiedergabefortschritt aktualisieren?",
            "Update your watch progress?",
          )}
        </h3>
        <p>{labelFor(pending, settings.uiLanguage)}</p>
        ...
        {mode === "choice" ? (
          <div className="external-playback-actions">
            <button
              ...
              className="primary"
              disabled={saving}
              onClick={() => void syncProgress(100, "stop")}
            >
              <CheckCircle2 size={18} />{" "}
              {localize(settings.uiLanguage, "Fertig", "Finished")}
            </button>
            <button
              type="button"
              className="secondary"
            ...
            <button
              ...
              className="secondary"
              disabled={saving}
              onClick={() => void syncProgress(1, "start")}
            >
              <Play size={18} fill="currentColor" />{" "}
              {localize(settings.uiLanguage, "Gerade begonnen", "Just started")}
            </button>
            <button
              type="button"
              className="ghost"
              ...
              onClick={() =>
                dismiss(
                  localize(
                    settings.uiLanguage,
                    "Externe Wiedergabe ignoriert.",
                    "External playback ignored.",
                  ),
                )
              }
            >
              {localize(settings.uiLanguage, "Nein", "No")}
            </button>
          </div>
        ) : (
          <>
            <div className="external-progress-grid">
              {[25, 50, 75, 90].map((progress) => (
                <button
                  ...
                  key={progress}
                  disabled={saving}
                  onClick={() => void syncProgress(progress, "pause")}
                >
                  {progress}%
                </button>
              ))}
        ...
        <small>
          {localize(
            settings.uiLanguage,
            "StreamNet kann die Wiedergabe externer Apps nicht auslesen und aktualisiert Trakt und StreamNet Cloud erst nach deiner Bestätigung.",
            "StreamNet cannot read playback from external apps and updates Trakt and StreamNet Cloud only after your confirmation.",
          )}
        </small>
      </article>
    </section>
