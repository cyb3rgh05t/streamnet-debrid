...
export function ProfileDialog({
  ...
  const [avatarColor, setAvatarColor] = useState(
    initial?.avatarColor ?? profileColors[0],
  );
  ...
  return (
    <div className="modal-scrim" onClick={onClose}>
      ...
      >
          ...
          <div className="avatar-tile lg">
            {avatarId > 0 ? (
              <div
                ...
                style={{
                  background: `linear-gradient(180deg, ${avatarGradient(avatarId)[0]}, ${avatarGradient(avatarId)[1]})`,
                }}
              >
                <img
              ...
              <div
                className="avatar-visual avatar-letter"
                style={{ background: colorToCss(avatarColor) }}
              >
        ...
        <h3 className="profile-dialog-label">
          {localize(
            settings.uiLanguage,
            "Farbe (Buchstabenavatar)",
            "Color (letter avatar)",
          )}
        </h3>
              ...
              <div className="avatar-grid">
                {category.ids.map((id) => (
                  <button
                    ...
                    style={{
                      background: `linear-gradient(180deg, ${avatarGradient(id)[0]}, ${avatarGradient(id)[1]})`,
                    }}
        ...
        <div className="profile-dialog-actions">
          {mode === "edit" && onDelete && (
            <button
              type="button"
              className="secondary text-button danger"
              onClick={onDelete}
            >
              <Trash2 size={18} />{" "}
              {localize(settings.uiLanguage, "Löschen", "Delete")}
            </button>
          )}
          <button
            type="button"
            className="primary"
            onClick={() =>
              onConfirm(name.trim() || "Profile", avatarColor, avatarId)
            }
            disabled={!name.trim()}
          >
            {mode === "add"
              ? localize(settings.uiLanguage, "Erstellen", "Create")
              : localize(settings.uiLanguage, "Speichern", "Save")}
          </button>
        </div>
      </div>
    </div>
