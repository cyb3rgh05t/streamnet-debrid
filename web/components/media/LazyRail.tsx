...
export function LazyRail({
  ...
  if (category) {
    return (
      <MediaRail
        category={{
          ...category,
          title: localizedCatalogName(catalog, settings.uiLanguage),
        }}
        onOpen={onOpen}
        onFocus={onFocus}
  ...
  return (
    <section
      ref={ref}
      className={`rail rail-skeleton ${effectivePosterMode ? "is-poster" : ""}`}
      aria-hidden
    >
...
function catalogCacheKey(catalog: CatalogConfig, language: string) {
  return [
    ...
    catalog.kind ?? "",
    catalog.collectionSources?.length
      ? JSON.stringify(catalog.collectionSources)
      : "",
  ].join("|");
}
...
function writeCachedCatalog(key: string, category: Category) {
  const cache = loadStored<CatalogRowCache>(CATALOG_ROW_CACHE_KEY, {});
  const next: CatalogRowCache = {
    ...cache,
    [key]: { at: Date.now(), category },
  };
