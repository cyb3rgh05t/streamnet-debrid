...
export function RailScroller({
  ...
  return (
    <div
      className={`rail-scroll-shell ${canPrev ? "can-prev" : ""} ${canNext ? "can-next" : ""}`}
    >
      <button
        type="button"
        ...
        aria-label={localize(
          settings.uiLanguage,
          `${ariaLabel} nach links scrollen`,
          `Scroll ${ariaLabel} left`,
        )}
      >
        <ChevronLeft size={24} />
      </button>
      ...
      <button
        ...
        aria-label={localize(
          settings.uiLanguage,
          `${ariaLabel} nach rechts scrollen`,
          `Scroll ${ariaLabel} right`,
        )}
      >
