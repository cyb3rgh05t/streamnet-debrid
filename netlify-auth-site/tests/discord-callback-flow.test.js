...
async function runCallback(search, response = { ok: true }) {
  ...
  const fetchCalls = [];
  const location = {
    href: `https://auth.arvio.tv/discord/callback${search}`,
    search
  };
  const document = {
    getElementById(id) {
...
test("mobile Discord callback returns to ARVIO without notifying the TV backend", async () => {
...
test("TV Discord callback notifies the backend without opening ARVIO on the phone", async () => {
  ...
  assert.equal(result.location.href, `https://auth.arvio.tv/discord/callback${search}`);
  assert.deepEqual(
    JSON.parse(result.fetchCalls[0][1].body),
    { device_code: deviceCode, code: "tv-code" }
  );
...
test("unmarked Discord state is rejected instead of guessing the authorization flow", async () => {
  ...
  assert.equal(result.fetchCalls.length, 0);
  assert.equal(result.location.href, `https://auth.arvio.tv/discord/callback${search}`);
  assert.equal(result.elements.get("heading").textContent, "Invalid Authorization Session");
});

test("mobile Discord errors return to ARVIO with the original state", async () => {
