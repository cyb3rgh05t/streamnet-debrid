...
const baseAddress = String(process.env.ARVIO_DELETION_TEST_EMAIL || "")
  .trim()
  .toLowerCase();
if (!baseAddress.includes("@")) {
  console.error(
    "Set ARVIO_DELETION_TEST_EMAIL to a real inbox that accepts plus aliases.",
  );
  process.exit(1);
}
...
const siteID =
  process.env.NETLIFY_SITE_ID || "17ccb668-bf3c-412d-9573-a28a09e52122";
const functionBase = "https://auth.streamnet.club/.netlify/functions";
const html = fs.readFileSync(
  new URL("../delete-account.html", import.meta.url),
  "utf8",
);
const appKey = html.match(/const APP_ANON_KEY = "([^"]+)"/)?.[1] || "";
if (!appKey)
  throw new Error("APP_ANON_KEY was not found in delete-account.html");
...
async function request(
  name,
  pathName,
  { method = "POST", headers = appHeaders, body, quiet = false } = {},
) {
  ...
  if (!quiet) {
    ...
    if (!response.ok) {
      const details =
        [result.error, result.message].filter(Boolean).join(" - ") || "unknown";
...
async function deletePrefix(store, prefix) {
  for await (const page of store.list({ prefix, paginate: true })) {
    await Promise.all(
      (page.blobs || []).map((blob) => store.delete(blob.key).catch(() => {})),
    );
  }
}
...
try {
  ...
  if (!accessToken || !refreshToken || !accountId)
    throw new Error("Synthetic session was incomplete");

  const claims = JSON.parse(
    Buffer.from(accessToken.split(".")[1], "base64url").toString("utf8"),
  );
  const storedAccount = await stores.auth.get(
    `accounts/email/${sha256(email)}.json`,
    {
      type: "json",
      consistency: "strong",
    },
  ...
  if (!identityMatches)
    throw new Error("Issued token does not match the stored account");
  ...
  if (!push.response.ok || !push.result.accepted)
    throw new Error("Synthetic sync push failed");
  ...
  if (started.response.status !== 202)
    throw new Error("Deletion did not queue");
  ...
  if (!deletionComplete)
    throw new Error(finalStatus?.error || "Deletion did not complete in time");
  ...
  const revocationPassed =
    loginAfter.response.status === 401 &&
