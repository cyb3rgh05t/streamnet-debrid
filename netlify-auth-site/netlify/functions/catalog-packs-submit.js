...
const { json, options, parseBody, getPool, sha256 } = require("./_backend");
...
async function resolvePublicAddresses(hostname) {
  ...
  if (ipaddr.isValid(cleaned)) {
    ...
    addresses = [
      { address: cleaned, family: parsed.kind() === "ipv6" ? 6 : 4 },
    ];
  } else {
    try {
  ...
  if (!Array.isArray(addresses) || addresses.length === 0) {
    throw new Error(
      `DNS lookup returned no addresses for hostname: ${hostname}`,
    );
  }
  if (addresses.some(({ address }) => isPrivateIp(address))) {
    throw new Error(
      "Access to private, loopback, or link-local IP addresses is blocked",
    );
  }
  return addresses;
}
      ...
      (response) => {
        ...
        response.on("data", (chunk) => {
          ...
          if (totalBytes > MAX_MANIFEST_BYTES) {
            fail(
              new Error("Response size exceeds the maximum limit of 512 KB"),
            );
            response.destroy();
            request.destroy();
            return;
...
async function fetchSecure(urlStr) {
  ...
  while (true) {
    ...
    if (
      ...
      response.body.length > MAX_MANIFEST_BYTES
    ) {
    ...
    try {
      ...
      throw new Error(
        "Failed to parse manifest as JSON. Please ensure it is valid JSON.",
      );
    }
  }
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!manifestUrl.startsWith("https://")) {
      return json(400, {
        error: "bad_request",
        message: "Manifest URL must start with https://",
      });
    }
    ...
    `,
      ...
      .catch((err) =>
        console.warn("Failed to create submission_rate_limits table", err),
      );
    ...
    if (
      ...
      return json(400, {
        ...
        message: "Manifest is missing a valid 'id'.",
      });
    }
    if (
      !manifest.name ||
    ...
    if (!Array.isArray(manifest.catalogs) || manifest.catalogs.length === 0) {
      return json(400, {
        error: "invalid_manifest",
        message: "Manifest must contain a non-empty 'catalogs' list.",
      });
    }
    ...
    for (const item of manifest.catalogs) {
      if (!item.name || typeof item.name !== "string" || !item.name.trim()) {
        return json(400, {
          error: "invalid_manifest",
          message:
            "All catalog items in the manifest must have a valid 'name'.",
        });
      }
