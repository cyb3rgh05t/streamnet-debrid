...
internal fun formatLiveFpsBadge(format: Format?): String? =
    format?.frameRate?.takeIf { it > 0f }?.let { value ->
        val rounded = kotlin.math.round(value).toInt()
        if (kotlin.math.abs(value - rounded) < 0.05f) "$rounded FPS" else "%.2f FPS".format(Locale.US, value)
    }

internal fun formatLiveAudioDetails(format: Format?, locale: Locale = Locale.getDefault()): String? {
    if (format == null) return null
    return formatLiveAudioDetails(
        codecSource = format.sampleMimeType ?: format.codecs,
...
internal fun formatLiveAudioDetails(
    codecSource: String?,
    channelCount: Int,
    languageTag: String?,
    ...
    val codec = when {
        codecSource == null -> null
        codecSource.contains("eac3", ignoreCase = true) -> "E-AC-3"
        codecSource.contains("ac3", ignoreCase = true) -> "AC-3"
        codecSource.contains("ac4", ignoreCase = true) -> "AC-4"
        codecSource.contains("truehd", ignoreCase = true) -> "TrueHD"
        codecSource.contains("dts", ignoreCase = true) -> "DTS"
        codecSource.contains("opus", ignoreCase = true) -> "Opus"
        codecSource.contains("vorbis", ignoreCase = true) -> "Vorbis"
        codecSource.contains("mp4a", ignoreCase = true) || codecSource.contains("aac", ignoreCase = true) -> "AAC"
        codecSource.contains("mpeg", ignoreCase = true) -> "MP3"
        else -> codecSource.substringAfterLast('/').uppercase(Locale.ROOT)
    }
    val channels = when (channelCount) {
        ...
        6 -> "5.1"
        8 -> "7.1"
        in 3..7 -> "$channelCount ch"
        else -> null
    }
    val language = languageTag
        ?.takeIf { it.isNotBlank() && !it.equals("und", ignoreCase = true) }
        ?.let { Locale.forLanguageTag(it.replace('_', '-')).getDisplayLanguage(locale) }
        ?.takeIf { it.isNotBlank() }
        ?.replaceFirstChar { if (it.isLowerCase()) it.titlecase(locale) else it.toString() }
    return listOfNotNull(codec, channels, language).takeIf { it.isNotEmpty() }?.joinToString(" · ")
}
...
private fun firstPlaylistCategoryId(tree: LiveCategoryTree): String {
    return tree.top.firstOrNull { it.id != "all" && it.id != "fav" && it.id != "recent" }?.id
        ?: tree.global.categories.firstOrNull()?.id
        ?: tree.countries.categories.firstOrNull()?.id
        ?: tree.adult.categories.firstOrNull()?.id
        ?: "all"
}
...
fun LiveTvScreen(
    ...
    val liveTvLayoutModeKey = remember(currentProfile?.id) {
        stringPreferencesKey("profile_${currentProfile?.id ?: "default"}_live_tv_layout_mode")
    }
    val liveTvLayoutMode by remember(context, liveTvLayoutModeKey) {
        context.settingsDataStore.data.map { preferences -> preferences[liveTvLayoutModeKey] ?: "streamnet" }
    }.collectAsStateWithLifecycle(initialValue = "streamnet")
    val liveTvNetflixTitleSizeKey = remember(currentProfile?.id) {
        stringPreferencesKey("profile_${currentProfile?.id ?: "default"}_live_tv_netflix_title_size")
    }
    val liveTvNetflixTitleSize by remember(context, liveTvNetflixTitleSizeKey) {
        context.settingsDataStore.data.map { preferences -> preferences[liveTvNetflixTitleSizeKey] ?: "Normal" }
    }.collectAsStateWithLifecycle(initialValue = "Normal")
    val liveTvNetflixDescriptionSizeKey = remember(currentProfile?.id) {
        stringPreferencesKey("profile_${currentProfile?.id ?: "default"}_live_tv_netflix_description_size")
    }
    val liveTvNetflixDescriptionSize by remember(context, liveTvNetflixDescriptionSizeKey) {
        context.settingsDataStore.data.map { preferences -> preferences[liveTvNetflixDescriptionSizeKey] ?: "Normal" }
    }.collectAsStateWithLifecycle(initialValue = "Normal")
    val liveTvClassicTitleSizeKey = remember(currentProfile?.id) {
        stringPreferencesKey("profile_${currentProfile?.id ?: "default"}_live_tv_classic_title_size")
    }
    val liveTvClassicTitleSize by remember(context, liveTvClassicTitleSizeKey) {
        context.settingsDataStore.data.map { preferences -> preferences[liveTvClassicTitleSizeKey] ?: "Normal" }
    }.collectAsStateWithLifecycle(initialValue = "Normal")
    val liveTvClassicDescriptionSizeKey = remember(currentProfile?.id) {
        stringPreferencesKey("profile_${currentProfile?.id ?: "default"}_live_tv_classic_description_size")
    }
    val liveTvClassicDescriptionSize by remember(context, liveTvClassicDescriptionSizeKey) {
    ...
    LaunchedEffect(hiddenGroupSet, selectedCategoryId, visibleEnrichedState.value.tree, state.config.showSpecialCategories) {
        val tree = visibleEnrichedState.value.tree
        val fallbackCategoryId = firstPlaylistCategoryId(tree)
        if (selectedCategoryId == "all" && !state.config.showSpecialCategories) {
    ...
    val previewInfoChannel = remember(
        ...
        val browsingGuide =
            focusZone == LiveTvFocusZone.CHANNEL_LIST || focusZone == LiveTvFocusZone.EPG
        if (!browsingGuide) return@remember playingChannel

        val targetId = selectedDisplayChannelId ?: focusedChannelId
        targetId
    ...
    var isFullScreen by if (isTouchDevice) {
        remember { mutableStateOf(initialStreamUrl != null) }
    } else {
        rememberSaveable { mutableStateOf(initialStreamUrl != null) }
    }
    ...
    // so app restarts reopen in the same Live TV context.
    LaunchedEffect(
        state.tvSessionLoaded,
        state.iptvPreferencesLoaded,
        selectedCategoryId,
    ...
    fun zapFullscreenChannel(direction: Int) {
        val channels = visibleChannels
        if (channels.isEmpty()) return
        val currentIndex = channels.indexOfFirst { it.id == playingChannelId }
            .takeIf { it >= 0 } ?: 0
        val targetIndex = (currentIndex + direction).mod(channels.size)
        val target = channels[targetIndex]
        if (target.id != playingChannelId) {
    ...
    LaunchedEffect(playingChannel?.id, playbackDiagnostic?.title, playbackDiagnostic?.channel?.id) {
        val resolvedChannel = playingChannel ?: return@LaunchedEffect
        val diagnostic = playbackDiagnostic ?: return@LaunchedEffect
        if (diagnostic.channel == null && diagnostic.severity != PlaybackDiagnosticSeverity.Error) {
            playbackDiagnostic = diagnostic.copy(
                detail = resolvedChannel.name,
    ...
    fun reloadCurrentLiveStream() {
        ...
        coroutineScope.launch {
            val reloadProgram = playingCatchupProgram
                ?.shiftedForCatchup(catchupUrlAnchorOffsetMs)
            ...
            if (playingChannelId != channelId) return@launch
            prepareStream(
                stream = target.url,
                isHls = target.isHls,
                headers = channel.requestHeaders,
                resetRetry = true,
                initialPositionMs = if (playingCatchupProgram != null) {
                    channel.catchupInSegmentSeekOffset(catchupPlaybackOffsetMs)
                } else {
                    0L
                },
        ...
        val listener = object : Player.Listener {
            override fun onPlaybackStateChanged(playbackState: Int) {
                ...
                if (playbackState == Player.STATE_READY) {
                    ...
                    val endedHeaders = endedChannel.requestHeaders
                    playerIsBuffering = true
                    coroutineScope.launch {
                        var resolveAttempt = 0
                        while (playingChannelId == endedChannelId && playingCatchupProgram == null) {
                            resolveAttempt++
                            ...
                            if (retryTarget != null) {
                                prepareStream(
                                    ...
                                    headers = endedHeaders,
                                    resetRetry = false,
                                    drmInfo = endedChannel.drmInfo,
                                    forcePrepare = true,
                                )
                                return@launch
                            }
            ...
            override fun onPlayerError(error: PlaybackException) {
                ...
                if (nextAttempt > maxRetryCount && retryProgram != null) {
                    playbackDiagnostic = PlaybackDiagnostic(
                        title = context.getString(R.string.live_diag_playback_failed),
                ...
                coroutineScope.launch {
                    val retryDelayMs = if (retryProgram == null && nextAttempt > maxRetryCount) {
                        10_000L
                    } else {
                        350L * nextAttempt
                    }
                    delay(retryDelayMs)
                    var retryTarget: IptvPlaybackTarget? = null
                    while (retryTarget == null &&
                        playingChannelId == playingChannel?.id &&
                        playingCatchupProgram == retryProgram
                    ) {
                        retryTarget = runCatching {
                    ...
                    val resolvedTarget = retryTarget ?: return@launch
                    System.err.println(
                        "[IPTV] Retrying live playback attempt=$nextAttempt " +
                            "code=${error.errorCodeName} status=${httpResponseCode(error) ?: "-"} " +
                            "candidates=$catchupCandidateCount url=${redactPlaybackUrl(resolvedTarget.url)}"
                    )
                    playbackDiagnostic = PlaybackDiagnostic(
                        title = context.getString(R.string.live_diag_retrying_source),
                    ...
                    prepareStream(
                        stream = resolvedTarget.url,
                        isHls = resolvedTarget.isHls,
    ...
    BackHandler(enabled = !searchOpen && isFullScreen && !fullscreenGuideOpen && !fullscreenSettingsOpen) {
        if (playingCatchupProgram != null) {
            returnCatchupToLive()
        } else {
    ...
    BackHandler(enabled = !searchOpen && isFullScreen && fullscreenSettingsOpen) {
        fullscreenSettingsOpen = false
        hudPokeSignal++
    }
    BackHandler(enabled = !searchOpen && variantPickerChannel == null && !isFullScreen) {
        if (useClassicLayout && !isTouchDevice) {
            when (focusZone) {
                LiveTvFocusZone.EPG -> focusChannelList(focusedChannelId ?: playingChannelId ?: guideDisplayChannels.firstOrNull()?.id)
    ...
    Box(
                ...
                if (!isTouchDevice) {
                    Modifier.onPreviewKeyEvent { event ->
                        ...
                        when (focusZone) {
                                ...
                                when (event.key) {
                                    ...
                                    Key.DirectionDown -> {
                                        if (!state.isConfigured && state.snapshot.channels.isEmpty()) {
                                            ...
                                            if (providerFilters.size > 1) focusProviderSwitcher() else focusClassicCategoryRail()
                                        } else {
                                            focusPlaylistSearch()
                                            focusSelectedCategorySignal += 1
                                        }
                            ...
                            LiveTvFocusZone.CHANNEL_LIST -> if (useClassicLayout) {
                                when (event.key) {
                                    Key.DirectionUp -> {
                                        val anchorId = focusedChannelId ?: selectedDisplayChannelId ?: playingChannelId
                                        val anchorIndex = guideDisplayChannels.indexOfFirst { it.id == anchorId }
                                            .takeIf { it >= 0 }
                                            ?: 0
                                        val targetIndex = anchorIndex - 1
                                        if (targetIndex >= 0) {
                                            focusChannelList(guideDisplayChannels[targetIndex].id)
                                        } else {
                                            focusClassicCategoryRail()
                                        }
                                        true
                                    ...
                                    Key.DirectionDown -> {
                                        ...
                                        val anchorIndex = guideDisplayChannels.indexOfFirst { it.id == anchorId }
                                            ...
                                            ?: 0
                                        val targetIndex = anchorIndex + 1
                                        if (targetIndex < guideDisplayChannels.size) {
                                            focusChannelList(guideDisplayChannels[targetIndex].id)
                                        } else {
        ...
        if (isFullScreen) {
            ...
            if (useClassicLayout) {
                LiveTvClassicLayout(
                    ...
                    onMoveProviderDown = { if (!isTouchDevice) focusClassicCategoryRail() else focusPlaylistSearch() },
                    previewDisplayChannel = previewDisplayChannel,
                    guideClockMillis = guideClockMillis,
                    previewNowNext = previewNowNext,
                    favoriteSet = favSet,
                    exoPlayer = exoPlayer,
                    ...
                    lookupBackdrop = { program ->
                        viewModel.lookupProgramBackdrop(program.title, program.startUtcMillis, program.endUtcMillis)
                    },
                    lookupLogo = { program ->
                        viewModel.lookupProgramLogo(program.title, program.startUtcMillis, program.endUtcMillis)
                    },
                    titleTextSize = liveTvClassicTitleSize,
                    descriptionTextSize = liveTvClassicDescriptionSize,
                    compactTouchLayout = compactTouchLayout,
                    landscapeCompactMiniPlayer = landscapeCompactMiniPlayer,
                    tabletLandscapeMiniPlayer = tabletLandscapeMiniPlayer,
                    ...
                    onMoveCategoryUp = { playlistId, groupName -> viewModel.moveGroupUp(playlistId, groupName) },
                    onMoveCategoryToTop = { playlistId, groupName -> viewModel.moveGroupToTop(playlistId, groupName) },
                    onMoveCategoryDown = { playlistId, groupName -> viewModel.moveGroupDown(playlistId, groupName) },
                    onCategoryFocusEnter = { focusZone = LiveTvFocusZone.CATEGORY_LIST },
                    onMoveCategoryRight = {
                    ...
                    focusSearchCategorySignal = focusSearchCategorySignal,
                    guideDisplayChannels = guideDisplayChannels,
                    ...
                    selectedDisplayChannelId = selectedDisplayChannelId,
                    ...
                    backHandlingEnabled = !searchOpen && variantPickerChannel == null,
                    ...
                    onMoveLeftFromChannels = { if (isTouchDevice) focusPlaylistSearch() else focusClassicCategoryRail() },
            ...
            Box(
                modifier = Modifier
                    ...
                    .onPreviewKeyEvent { ev ->
                        ...
                        if (fullscreenSettingsOpen) {
                            ...
                            if (firstPress) {
                                if (ev.nativeKeyEvent.keyCode == AndroidKeyEvent.KEYCODE_MENU) {
                                ...
                                // With the HUD hidden, only Up/Down changes channels.
                                // With it visible, directional keys stay with Compose focus navigation.
                                if (!isHudVisible && (ev.key == Key.DirectionUp || ev.key == Key.DirectionDown)) {
                                    zapFullscreenChannel(if (ev.key == Key.DirectionDown) 1 else -1)
                                    return@onPreviewKeyEvent true
                                }
                                if (!isHudVisible) {
                                    if (ev.key in listOf(Key.DirectionCenter, Key.Enter)) {
                                        hudPokeSignal++
                                        return@onPreviewKeyEvent true
                                    }
                                }
                    ...
                    .then(
                        ...
                        } else if (isFullScreen && !fullscreenGuideOpen && !quickZapOpen && !fullscreenSettingsOpen) {
                            Modifier.onPreviewKeyEvent { ev ->
                                val isHiddenHudChannelKey = !isHudVisible &&
                                    (ev.key == Key.DirectionUp || ev.key == Key.DirectionDown)
                                if (ev.type == KeyEventType.KeyDown && !isHiddenHudChannelKey) {
                                    hudPokeSignal++
                                }
                                false
                            }
                ...
                androidx.compose.ui.viewinterop.AndroidView(
                    ...
                    update = { view ->
                        ...
                        view.resizeMode = liveTvResizeMode
                    },
                ...
                if (isFullScreen && !fullscreenGuideOpen && !quickZapOpen) {
                    ...
                    FullscreenHud(
                        ...
                        streamBadges = listOfNotNull(
                            ...
                            formatLiveFpsBadge(currentVideoFormat),
                        ),
                        streamInfoLines = listOf(
                            stringResource(R.string.live_player_protocol) to if (lastPreparedIsHls) "HLS" else "MPEG-TS",
                            stringResource(R.string.live_player_resolution) to currentVideoFormat?.let { format ->
                                if (format.width > 0 && format.height > 0) "${format.width} × ${format.height}" else "-"
                            }.orEmpty().ifBlank { "-" },
                            stringResource(R.string.live_player_video_codec) to (
                                currentVideoFormat?.codecs
                                    ?: currentVideoFormat?.sampleMimeType?.substringAfterLast('/')
                                    ?: "-"
                                ),
                            stringResource(R.string.live_player_audio_codec) to (
                                formatLiveAudioDetails(currentAudioFormat) ?: "-"
                                ),
                        ),
                        onSettingsClick = {
                            fullscreenSettingsOpen = true
        ...
        // Rebuild the media item so prepare is not a no-op on an already prepared player.
