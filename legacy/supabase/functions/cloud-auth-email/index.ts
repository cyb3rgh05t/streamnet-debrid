...
function pruneSignupAttempts(now: number) {
  ...
  for (const [email, timestamp] of emailSignupAttempts.entries()) {
    if (now - timestamp > emailSignupCooldownMs)
      emailSignupAttempts.delete(email);
  }
}
...
function enforceSignupRateLimit(req: Request, email: string): string | null {
  ...
  if (
    ...
    existing.count >= maxSignupAttemptsPerIp
  ) {
...
serve(async (req) => {
  ...
  try {
    ...
    const createResp = await createConfirmedUser(
      supabaseUrl,
      serviceRole,
      email,
      password,
    ...
    const tokenResp = await passwordToken(
      supabaseUrl,
      anonKey,
      email,
      password,
