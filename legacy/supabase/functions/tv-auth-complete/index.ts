...
function validateEmail(email: string, rejectDisposable = true): string | null {
  ...
  if (
    ...
    domainLabels.some((part) => !part || part.length > 63)
  ) {
    return "Enter a valid email address";
  }
  ...
  if (rejectDisposable && blockedSignupLocalParts.has(localPart))
    return "Use a real email address";
  if (rejectDisposable && isBlockedEmailDomain(domain))
    return "Use a real email address";
  if (
    rejectDisposable &&
...
function enforceEmailSendCooldown(
  action: string,
  email: string,
): string | null {
...
function parseAuthError(raw: string): string {
  try {
    ...
    return String(
      json.error_description || json.msg || json.message || json.error || raw,
    );
  } catch {
    return raw;
  }
...
serve(async (req) => {
  ...
  try {
    ...
    if (intent === "signup") {
      const signupResp = await createConfirmedUser(
        supabaseUrl,
        serviceRole,
        email,
        password,
