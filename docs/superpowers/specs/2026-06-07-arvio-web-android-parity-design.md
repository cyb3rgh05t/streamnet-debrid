# StreamNet TV Web ↔ Android Parity — Design
...
| 8 — Visual + i18n + QA | ✅ Mostly | Poster mode, accent themes, OLED, clock format, smooth scrolling, spoiler blur all applied live; full-screen QA across every screen, zero console errors. **Deferred:** 50-language _UI-chrome_ translation (TMDB _content_ already respects the language setting); fine-grained pixel matching. |
...
| --------------- | ------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **UI fidelity** | Screen + feature parity (not pixel-perfect) | Every screen, setting, catalog and backend present and behaving the same; styled to closely match the StreamNet TV look; minor web-native polish (scrollbars, hover, focus) allowed.                            |
