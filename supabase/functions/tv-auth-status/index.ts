...
serve(async (req) => {
  ...
  try {
    ...
    const rows = (await queryResponse.json()) as Array<
    ...
    if (isExpired && row.status === "pending") {
      await fetch(
        `${supabaseUrl}/rest/v1/tv_device_auth_sessions?device_code=eq.${encodeURIComponent(deviceCode)}`,
        {
          method: "PATCH",
          headers: {
            apikey: serviceRole,
      ...
      return new Response(
        JSON.stringify({ status: "expired", message: "Code expired" }),
        {
          headers: { ...corsHeaders(req), "Content-Type": "application/json" },
        },
      );
