...
function pruneResetAttempts(now: number) {
  ...
  for (const [email, timestamp] of emailResetAttempts.entries()) {
    if (now - timestamp > emailResetCooldownMs)
...
function enforceResetRateLimit(req: Request, email: string): string | null {
  ...
  if (
    ...
    existing.count >= maxResetAttemptsPerIp
  ) {
...
serve(async (req) => {
  ...
  try {
    ...
    const redirectTo =
      body.redirect_to?.trim() || "https://auth.streamnet.club/?mode=recovery";
    ...
    if (!resetResp.ok) {
      ...
      return new Response(
        JSON.stringify({
          error: parseAuthError(resetText) || "Password reset failed",
        }),
        {
